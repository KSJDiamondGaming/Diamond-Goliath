...
const replacement = `  const identityLines = [
    \`Username: \\\`${target.user.username}\\\` • Global: ${target.user.globalName || 'None'}\`,
    \`Display: ${target.displayName || target.user.username} • Bot: ${target.user.bot ? 'Yes' : 'No'}\`,
    \`Created: ${scanTimestamp(target.user.createdTimestamp)}\`,
  ];
  if (access.history) identityLines.push(historicalNames.length
    ? \`Previous: ${historicalNames.slice(0, 8).map((name) => \\\`\\\`${name}\\\`\\\`).join(' • ')} • ${history.scanCount} prior scan(s)\`
  ...
  const membershipLines = [
    ...
    \`Roles (${roles.length}): ${(roles.slice(0, 10).map((role) => \\\`${role}\\\`).join(', ') || 'None').slice(0, 500)}\`,
