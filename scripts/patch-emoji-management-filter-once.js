...
const filterHandler = "  if (id.startsWith('admin:module:emojis:manage-filter:') && interaction.isButton?.()) { setManageFilter(interaction, id.slice('admin:module:emojis:manage-filter:'.length)); await sendPanel(interaction, managePanel(await discordOverview(interaction), interaction)); return true; }\n";
source = source.replace(interactionAnchor, interactionAnchor + filterHandler);
