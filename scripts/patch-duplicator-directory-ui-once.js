...
core = replaceRange(core, 'async function getGuildDirectory(client)', 'async function refreshSessionDirectory', lines([
...
const oldCopyActions = "  else if (data.action === 'conflict') session.conflictMode = interaction.values?.[0] || 'skip';\n  else if (data.action === 'cancel')";
const newCopyActions = "  else if (data.action === 'conflict') session.conflictMode = interaction.values?.[0] || 'skip';\n  else if (data.action === 'refresh') { await refreshSessionDirectory(interaction.client, session); session.expiresAt = Date.now() + SESSION_TTL_MS; }\n  else if (data.action === 'dryrun') session.dryRun = !session.dryRun;\n  else if (data.action === 'cancel')";
if (!core.includes(oldCopyActions)) throw new Error('copy action marker missing');
core = core.replace(oldCopyActions, newCopyActions);

const oldAnalyseActions = "  if (data.action === 'source') session.sourceGuildId = interaction.values?.[0];\n  else if (data.action === 'destination') session.destinationGuildId = interaction.values?.[0];\n  else if (data.action === 'start') {";
const newAnalyseActions = "  if (data.action === 'source') session.sourceGuildId = interaction.values?.[0];\n  else if (data.action === 'destination') session.destinationGuildId = interaction.values?.[0];\n  else if (data.action === 'refresh') { await refreshSessionDirectory(interaction.client, session); session.expiresAt = Date.now() + SESSION_TTL_MS; }\n  else if (data.action === 'cancel') { analyseSessions.delete(session.id); return interaction.update({ embeds: [embed('❌ Analyse Cancelled', 'No changes were made.', 0xef4444)], components: [] }); }\n  else if (data.action === 'start') {";
if (!core.includes(oldAnalyseActions)) throw new Error('analyse action marker missing');
core = core.replace(oldAnalyseActions, newAnalyseActions);
