...
function TextField({ theme, label, value, onChange, placeholder = '' }) {
  return (
    <label style={{ display: 'grid', gap: 8 }}>
      <span style={{ color: theme.mutedText, fontSize: 12, fontWeight: 900, textTransform: 'uppercase' }}>{label}</span>
      <input value={value || ''} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} style={{ border: `1px solid ${theme.cardBorder}`, background: 'rgba(15,23,42,0.55)', color: theme.cardText, borderRadius: 14, padding: '12px 14px', fontWeight: 800 }} />
    </label>
  );
}
  ...
  return (
    <div style={{ display: 'grid', gap: 18 }}>
      <section style={{ ...card, background: 'linear-gradient(135deg, rgba(52,211,153,0.18), rgba(15,23,42,0.08) 48%, rgba(59,130,246,0.14))' }}>
        ...
        <p style={{ color: theme.mutedText, lineHeight: 1.6 }}>Configure public welcome messages, direct messages, member pings, templates, testing, health and analytics.</p>
      </section>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,170px),1fr))', gap: 12 }}>
        ...
        <Stat theme={theme} label="Skipped" value={analytics.skipped || 0} />
      ...
      <section style={{ ...card, display: 'grid', gap: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div><h2 style={{ margin: 0 }}>Module Setup</h2><p style={{ margin: '6px 0 0', color: theme.mutedText }}>All settings are saved through GuildManager.</p></div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,240px),1fr))', gap: 14 }}>
          <TextField theme={theme} label="Public Template ID" value={config?.templateId || ''} onChange={(value) => setConfig((current) => ({ ...(current || {}), templateId: value }))} placeholder="welcome_default" />
          <TextField theme={theme} label="DM Template ID" value={config?.dmTemplateId || ''} onChange={(value) => setConfig((current) => ({ ...(current || {}), dmTemplateId: value }))} placeholder="dm_welcome_default" />
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <button type="button" disabled={busy} onClick={() => savePatch({ templateId: config?.templateId, dmTemplateId: config?.dmTemplateId })} style={buttonStyle(theme, 'success')}>Save Templates</button>
          <button type="button" disabled={busy} onClick={() => act('test', () => api.request(`/api/welcome/${guildId}/test`, { method: 'POST', body: JSON.stringify({ userId: selectedGuildData?.userId }) }), 'Test welcome sent.')} style={buttonStyle(theme, 'success')}>Test Welcome</button>
