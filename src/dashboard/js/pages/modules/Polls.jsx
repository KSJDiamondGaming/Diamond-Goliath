...
const pollsApi = {
  ...
  settings: (guildId, settings) => api.request(`/api/polls/${guildId}/settings`, { method: 'PATCH', body: JSON.stringify({ settings }) }),
...
export default function Polls({ theme, selectedGuild, selectedGuildData }) {
  ...
  async function load() {
    ...
    try {
      ...
      setSettings({
        defaultChannelId: nextConfig?.settings?.defaultChannelId || '',
        autoCloseHours: Number(nextConfig?.settings?.autoCloseHours || 0),
        allowMultipleVotes: nextConfig?.settings?.allowMultipleVotes === true,
        anonymousVotes: nextConfig?.settings?.anonymousVotes === true,
        showResultsLive: nextConfig?.showResultsLive !== false,
      });
    } catch (loadError) {
  ...
  function createPoll() {
    return run(() => pollsApi.create(guildId, {
      question: draft.question,
      description: draft.description,
      channelId: draft.channelId,
      options: draft.options.filter((value) => value.trim()).map((label) => ({ label })),
    }), 'Poll created.').then(() => setDraft({ question: '', description: '', channelId: settings.defaultChannelId || '', options: ['Option 1', 'Option 2'] }));
  }
  ...
  const overviewContent = (
    <div style={{ display: 'grid', gap: 16 }}>
      <StatGrid min="160px">
        ...
        <SummaryStat theme={theme} label="Responses" value={overview?.responses || 0} accent="#f59e0b" description="Recorded votes" />
      </StatGrid>
      <SectionCard theme={theme} title="Saved Polls" subtitle="Create, deploy, refresh, close and remove polls.">
        <div style={{ display: 'grid', gap: 10 }}>
          {polls.length ? polls.map((poll) => <PollCard key={poll.id} theme={theme} poll={poll} saving={saving} onDeploy={(item) => run(() => pollsApi.deploy(guildId, item.id, { channelId: item.channelId || settings.defaultChannelId }), 'Poll deployed or refreshed.')} onClose={(item) => run(() => pollsApi.status(guildId, item.id, 'closed'), 'Poll closed.')} onDelete={(item) => run(() => pollsApi.remove(guildId, item.id), 'Poll deleted.')} />) : <EmptyState theme={theme} title="No polls yet" text="Create a poll to deploy it to Discord." />}
        </div>
      </SectionCard>
    </div>
  );
  ...
  const configurationContent = (
    <div style={{ display: 'grid', gap: 16 }}>
      <SectionCard theme={theme} title="Module Settings" subtitle="Control voting defaults and automatic closing.">
        <div style={{ display: 'grid', gap: 12 }}>
          {field(theme, 'Default Channel', <Select value={settings.defaultChannelId} onChange={(event) => setSettings({ ...settings, defaultChannelId: event.target.value })}><option value="">Select a channel</option>{channels.map((channel) => <option key={channel.id} value={channel.id}>#{channel.name}</option>)}</Select>)}
          {field(theme, 'Auto-close after hours (0 disables)', <Input type="number" min="0" max="8760" value={settings.autoCloseHours} onChange={(event) => setSettings({ ...settings, autoCloseHours: Number(event.target.value) })} />)}
          <Toggle label="Allow multiple choices" checked={settings.allowMultipleVotes} onChange={(value) => setSettings({ ...settings, allowMultipleVotes: value })} />
          <Toggle label="Anonymous results" checked={settings.anonymousVotes} onChange={(value) => setSettings({ ...settings, anonymousVotes: value })} />
          <Toggle label="Show results live" checked={settings.showResultsLive} onChange={(value) => setSettings({ ...settings, showResultsLive: value })} />
          <PrimaryButton onClick={() => run(() => pollsApi.settings(guildId, settings), 'Poll settings saved.')} disabled={saving}>Save Settings</PrimaryButton>
        </div>
      </SectionCard>
      <SectionCard theme={theme} title="Create Poll" subtitle="Create a poll record before deploying it to Discord.">
        <div style={{ display: 'grid', gap: 12 }}>
          ...
          {field(theme, 'Channel', <Select value={draft.channelId} onChange={(event) => setDraft({ ...draft, channelId: event.target.value })}><option value="">Use default channel</option>{channels.map((channel) => <option key={channel.id} value={channel.id}>#{channel.name}</option>)}</Select>)}
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><SecondaryButton theme={theme} onClick={() => setDraft({ ...draft, options: [...draft.options, `Option ${draft.options.length + 1}`] })} disabled={draft.options.length >= 10}>Add Option</SecondaryButton><SecondaryButton theme={theme} onClick={() => setDraft({ ...draft, options: draft.options.slice(0, Math.max(2, draft.options.length - 1)) })} disabled={draft.options.length <= 2}>Remove Option</SecondaryButton></div>
          <PrimaryButton onClick={createPoll} disabled={saving || !draft.question.trim() || draft.options.filter((value) => value.trim()).length < 2}>{saving ? 'Saving...' : 'Create Poll'}</PrimaryButton>
        </div>
      </SectionCard>
    </div>
  );
  ...
  const discordExperienceContent = (
    <SectionCard theme={theme} title="Discord Administration" subtitle="The Discord admin panel now supports full poll creation and management.">
      <p style={{ margin: 0, color: theme.mutedText, lineHeight: 1.6 }}>Administrators can create polls with a modal, select stored polls, deploy or refresh messages, close voting, delete polls, inspect health, repair deployments, export configuration and reset the module.</p>
    </SectionCard>
  );

  const activityContent = (
    <div style={{ display: 'grid', gap: 16 }}>
      <SectionCard theme={theme} title="Health" subtitle="Validate active Discord poll deployments.">
        <div style={{ display: 'grid', gap: 10, color: theme.mutedText, fontWeight: 800 }}><span>Status: {health?.healthy ? 'Healthy' : 'Needs attention'}</span><span>Issues: {health?.issues?.length || 0}</span>{(health?.issues || []).slice(0, 8).map((issue, index) => <span key={`${issue.code}-${index}`}>• {issue.code}{issue.pollId ? ` — ${issue.pollId}` : ''}</span>)}</div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 12 }}><PrimaryButton onClick={() => run(() => pollsApi.repair(guildId), 'Poll deployments repaired.')} disabled={saving}>Repair</PrimaryButton><SecondaryButton theme={theme} onClick={exportConfig} disabled={saving}>Export</SecondaryButton><button type="button" disabled={saving} onClick={() => { if (window.confirm('Delete tracked poll messages and reset Polls?')) run(() => pollsApi.reset(guildId), 'Polls reset.'); }} style={{ border: '1px solid rgba(248,113,113,0.35)', background: 'rgba(248,113,113,0.12)', color: '#fca5a5', borderRadius: 12, padding: '10px 12px', fontWeight: 900 }}>Reset</button></div>
      </SectionCard>
    </div>
  );
