...
export default function SecurityCenter({ theme }) {
  ...
  const selectedGuildName =
    guilds.find((guild) => String(guild.guildId || guild.id) === selectedGuild)?.name ||
    guilds.find((guild) => String(guild.guildId || guild.id) === selectedGuild)?.guildName ||
    'No guild selected';
  ...
  return (
    <div style={{ display: 'grid', gap: 18 }}>
      <section style={card}>
        <p style={{ margin: 0, color: '#ef4444', fontWeight: 900, textTransform: 'uppercase' }}>
          Global Security
        </p>
        ...
        <p style={{ marginTop: 8, color: theme.mutedText }}>
          View incidents, lockdowns, quarantines and protection events across all environments.
        </p>
      </section>
      ...
      >
        ...
        <SecurityCard title="Quarantined Users" value={String(security?.quarantines?.length || 0)} theme={theme} />
        <SecurityCard title="Security Incidents" value={String(incidents.length)} theme={theme} />
      </section>
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            {incidents.map((incident, index) => (
              ...
              >
                <div>
                  <strong>{incident.type || 'Security Event'}</strong>
                  <div style={{ color: theme.mutedText }}>
                    {incident.environment || incident.guildId || selectedGuildName}
                  </div>
                </div>
