...
router.patch('/:guildId/enabled', async (req, res) => { try { const guildId = getGuildId(req); const enabled = req.body?.enabled === true; tempVoiceStore.setEnabled(guildId, enabled, { actorId: req.body?.actorId, action: 'temp_voice_toggle' }); const config = canonicalConfig(guildId); const guild = await fetchGuild(req, guildId); return success(res, { guildId, config, overview: overview(guildId, config, guild) }); } catch (error) { return failure(res, error, 400); } });
