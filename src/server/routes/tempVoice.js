...
function assertTempVoiceEnabled(guildId) { if (!isModuleEnabled(guildId, 'tempVoice')) throw new Error('Temp Voice module is disabled for this server.'); }
...
router.get('/:guildId', async (req, res) => { try { const guildId = getGuildId(req); const guild = await fetchGuild(req, guildId); const config = tempVoiceStore.getTempVoiceSection(guildId); return success(res, { guildId, config, overview: overview(config, guild) }); } catch (error) { return failure(res, error, 400); } });
router.get('/:guildId/health', async (req, res) => { try { const guildId = getGuildId(req); const guild = await fetchGuild(req, guildId); if (!guild) throw new Error('Guild is not available to the bot.'); const config = tempVoiceStore.getTempVoiceSection(guildId); const health = await scanTempVoiceOrphans(guild, config); return success(res, { guildId, health, config, overview: overview(config, guild) }); } catch (error) { return failure(res, error, 400); } });
router.post('/:guildId/cleanup-orphans', async (req, res) => { try { const guildId = getGuildId(req); const guild = await fetchGuild(req, guildId); if (!guild) throw new Error('Guild is not available to the bot.'); const section = tempVoiceStore.getTempVoiceSection(guildId); const cleanup = await cleanupTempVoiceOrphans(guild, section, req.body || {}); const config = tempVoiceStore.saveTempVoiceSection(guildId, { ...cleanup.section, updatedAt: tempVoiceStore.now() }, { actorId: req.body?.actorId, action: 'temp_voice_cleanup_orphans' }); const health = await scanTempVoiceOrphans(guild, config); return success(res, { guildId, cleanup: cleanup.result, health, config, overview: overview(config, guild) }); } catch (error) { return failure(res, error, 400); } });
router.patch('/:guildId/enabled', async (req, res) => { try { const guildId = getGuildId(req); const enabled = req.body?.enabled === true; setModuleEnabled(guildId, 'tempVoice', enabled); const config = tempVoiceStore.updateTempVoiceSection(guildId, (section) => ({ ...section, enabled, updatedAt: tempVoiceStore.now() }), { actorId: req.body?.actorId }); const guild = await fetchGuild(req, guildId); return success(res, { guildId, config, overview: overview(config, guild) }); } catch (error) { return failure(res, error, 400); } });
router.patch('/:guildId/settings', async (req, res) => { try { const guildId = getGuildId(req); const settings = prepareSettings(req.body?.settings || req.body || {}); const config = tempVoiceStore.updateTempVoiceSection(guildId, (section) => ({ ...section, settings: { ...(section.settings || {}), ...settings }, updatedAt: tempVoiceStore.now() }), { actorId: req.body?.actorId }); const guild = await fetchGuild(req, guildId); return success(res, { guildId, config, overview: overview(config, guild) }); } catch (error) { return failure(res, error, 400); } });
router.post('/:guildId/hubs/deploy', async (req, res) => { try { const guildId = getGuildId(req); const guild = await fetchGuild(req, guildId); if (!guild) throw new Error('Guild is not available to the bot.'); const deployment = await tempVoiceManager.deployHub(guild, prepareHub(req.body || {})); const config = tempVoiceStore.getTempVoiceSection(guildId); return success(res, { guildId, hub: deployment.hub || deployment, deployment, config, overview: overview(config, guild) }); } catch (error) { return failure(res, error, 400); } });
router.post('/:guildId/hubs', async (req, res) => { try { const guildId = getGuildId(req); const hub = tempVoiceManager.createHub(guildId, prepareHub(req.body || {})); const config = tempVoiceStore.getTempVoiceSection(guildId); const guild = await fetchGuild(req, guildId); return success(res, { guildId, hub, config, overview: overview(config, guild) }); } catch (error) { return failure(res, error, 400); } });
router.put('/:guildId/hubs/:hubId', async (req, res) => { try { const guildId = getGuildId(req); assertTempVoiceEnabled(guildId); const guild = await fetchGuild(req, guildId); const beforeConfig = tempVoiceStore.getTempVoiceSection(guildId); const beforeHub = beforeConfig.hubs?.[req.params.hubId] || null; if (!beforeHub) throw new Error('Temp Voice hub was not found.'); const nextHub = tempVoiceStore.saveHub(guildId, prepareHub({ ...(req.body || {}), hubId: req.params.hubId }), { actorId: req.body?.actorId }); const sync = await syncHubToDiscord(guild, beforeHub, nextHub); const config = tempVoiceStore.getTempVoiceSection(guildId); return success(res, { guildId, hub: nextHub, sync, config, overview: overview(config, guild) }); } catch (error) { return failure(res, error, 400); } });
router.delete('/:guildId/hubs/:hubId', async (req, res) => { try { const guildId = getGuildId(req); assertTempVoiceEnabled(guildId); const guild = await fetchGuild(req, guildId); const beforeConfig = tempVoiceStore.getTempVoiceSection(guildId); const hub = beforeConfig.hubs?.[req.params.hubId] || null; if (!hub) throw new Error('Temp Voice hub was not found.'); const sync = await deleteHubDiscordResources(guild, beforeConfig, req.params.hubId, hub); const config = tempVoiceStore.updateTempVoiceSection(guildId, (section) => { const hubs = { ...(section.hubs || {}) }; const channels = { ...(section.channels || {}) }; delete hubs[req.params.hubId]; for (const [channelId, channel] of Object.entries(channels)) if (channel.hubId === req.params.hubId || channel.hubId === hub.hubId) delete channels[channelId]; return { ...section, hubs, channels, updatedAt: tempVoiceStore.now() }; }, { actorId: req.body?.actorId }); return success(res, { guildId, sync, config, overview: overview(config, guild) }); } catch (error) { return failure(res, error, 400); } });
router.patch('/:guildId/channels/:channelId/controls', async (req, res) => { try { const guildId = getGuildId(req); const guild = await fetchGuild(req, guildId); if (!guild) throw new Error('Guild is not available to the bot.'); const channel = await tempVoiceManager.updateTempChannelControls(guild, req.params.channelId, getActorId(req), prepareChannelControls(req.body?.controls || req.body || {})); const config = tempVoiceStore.getTempVoiceSection(guildId); return success(res, { guildId, channel, config, overview: overview(config, guild) }); } catch (error) { return failure(res, error, 400); } });
router.post('/:guildId/channels/:channelId/claim', async (req, res) => { try { const guildId = getGuildId(req); const guild = await fetchGuild(req, guildId); if (!guild) throw new Error('Guild is not available to the bot.'); const channel = await tempVoiceManager.claimTempChannel(guild, req.params.channelId, getActorId(req)); const config = tempVoiceStore.getTempVoiceSection(guildId); return success(res, { guildId, channel, config, overview: overview(config, guild) }); } catch (error) { return failure(res, error, 400); } });
router.post('/:guildId/channels/:channelId/kick', async (req, res) => { try { const guildId = getGuildId(req); const guild = await fetchGuild(req, guildId); const targetId = cleanId(req.body?.targetId || req.body?.userId); if (!guild) throw new Error('Guild is not available to the bot.'); if (!targetId) throw new Error('Target user ID is required.'); const channel = await tempVoiceManager.kickMemberFromTempChannel(guild, req.params.channelId, getActorId(req), targetId, req.body?.block === true); const config = tempVoiceStore.getTempVoiceSection(guildId); return success(res, { guildId, channel, config, overview: overview(config, guild) }); } catch (error) { return failure(res, error, 400); } });
router.delete('/:guildId/channels/:channelId', async (req, res) => { try { const guildId = getGuildId(req); assertTempVoiceEnabled(guildId); const guild = await fetchGuild(req, guildId); if (guild && getActorId(req)) await tempVoiceManager.deleteOwnedTempChannel(guild, req.params.channelId, getActorId(req)); else { tempVoiceStore.deleteTempChannel(guildId, req.params.channelId, { actorId: req.body?.actorId }); const channel = guild?.channels?.cache?.get(req.params.channelId) || await guild?.channels?.fetch?.(req.params.channelId).catch(() => null); if (channel?.deletable) await channel.delete('Goliath Temp Voice dashboard delete').catch(() => null); } const config = tempVoiceStore.getTempVoiceSection(guildId); return success(res, { guildId, config, overview: overview(config, guild) }); } catch (error) { return failure(res, error, 400); } });
