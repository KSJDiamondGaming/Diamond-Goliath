...
function section(guildId) {
  return { ...base(), ...obj(getGuildSection(guildId, 'notifications', base())) };
}
...
function markRead(guildId, notificationId, read = true) {
  ...
  updateGuildSection(guildId, 'notifications', (current = base()) => {
    const next = { ...base(), ...obj(current) };
    const notifications = (next.notifications || []).map((item) => {
...
function markAllRead(guildId) {
  updateGuildSection(guildId, 'notifications', (current = base()) => {
    const next = { ...base(), ...obj(current) };
    return { ...next, notifications: (next.notifications || []).map((item) => ({ ...normalise(item), read: true, updatedAt: now() })), updatedAt: now() };
  }, base());
