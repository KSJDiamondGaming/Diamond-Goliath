// src/core/systems/security/restoreBackup/worker.js
...
async function processPendingSyncs() {
  if (workerRunning) {
    return {
      skipped: true,
      reason: 'Sync worker already running.',
    };
  }
...
function startBackupWorker(options = {}) {
  const intervalMs = Number(options.intervalMs || process.env.BACKUP_SYNC_INTERVAL_MS) || DEFAULT_INTERVAL_MS;
