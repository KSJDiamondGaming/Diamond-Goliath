...
async function requestRemoveTimeout(interaction, targetId) {
  ...
  if (!target) return true;
  return createConfirmation(interaction, target.id, 'remove-timeout', {}, `✅ Remove timeout from **${target.user.tag}**?`);
}
...
async function handleActionSelectMenu(interaction) {
  ...
  return routeActionRequest(interaction, interaction.values[0], getTargetIdFromCustomId(interaction.customId));
}
...
async function handleCaseToolButton(interaction) {
  ...
  if (id.startsWith('mod_remove_timeout:')) return routeActionRequest(interaction, 'remove-timeout', targetId);
  return false;
}
...
async function handleBulkButton(interaction) {
  ...
  if (!action) return false;
  const allowed = await ensureActionAccess(interaction, `bulk_${action}`, `❌ No permission to use bulk ${action}.`);
...
async function handleActionModal(interaction) {
  const id = String(interaction.customId || '');
  const targetId = getTargetIdFromCustomId(id);
  if (id.startsWith('mod_submit_warn:')) return submitWarningModal(interaction, targetId, refreshCasesDashboard);
  if (id.startsWith('mod_submit_remove_warning:')) return submitRemoveWarningRequest(interaction, targetId, createConfirmation);
  const action = getPunishmentSubmitAction(id);
  if (!action) return false;
  const target = await requireModeratableTarget(interaction, targetId, action);
  if (!target) return true;
  const result = await submitPunishmentRequest(interaction, target, action);
  if (action === 'timeout' && result?.ok) await refreshCasesDashboard(interaction, target);
  return true;
}
