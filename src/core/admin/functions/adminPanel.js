...
function getAutomodConfig(gid){ const cur=getGuildSection(gid,'automod',{}), d=defaults(), out={...d,...cur}; for(const k of AUTOMOD_RULE_KEYS){ const e=cur[k]||{}; out[k]={...d[k],...e,actions:normalizeActions(e.actions||e.action,d[k].actions)}; delete out[k].action; } out.dmMessages={...DEFAULT_DM_MESSAGES,...(cur.dmMessages||{})}; out.ignoredRoles=Array.isArray(cur.ignoredRoles)?cur.ignoredRoles:[]; out.ignoredChannels=Array.isArray(cur.ignoredChannels)?cur.ignoredChannels:[]; return out; }
...
const parseList=v=>[...new Set(String(v||'').split(',').map(x=>x.trim().toLowerCase()).filter(Boolean))].slice(0,100);
...
async function handleAutomodModal(i){
  ...
  if(k==='antiLinks'){r.allowStaff=i.fields.getTextInputValue('allowStaff').trim().toLowerCase()!=='false';r.allowedDomains=parseList(i.fields.getTextInputValue('allowedDomains'));}
