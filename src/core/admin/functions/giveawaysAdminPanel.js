...
const giveawaysStore = require('../../../modules/giveaways/giveawaysStore');
const giveawaysManager = require('../../../modules/giveaways/giveawaysManager');
...
function buildGiveawaysAdminPanel(guild, memberDisplayName = 'Unknown User') {
  ...
  return {
    ...
    components: [
      row(
        new ChannelSelectMenuBuilder().setCustomId('admin:giveaways:announcementChannel').setPlaceholder('Announcement channel').setChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement).setMinValues(0).setMaxValues(1)
      ),
      row(
        new ChannelSelectMenuBuilder().setCustomId('admin:giveaways:logChannel').setPlaceholder('Log channel').setChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement).setMinValues(0).setMaxValues(1)
      ),
      row(
        new RoleSelectMenuBuilder().setCustomId('admin:giveaways:managerRoles').setPlaceholder('Manager roles').setMinValues(0).setMaxValues(10)
      ),
      row(
        button('admin:giveaways:deployTest', '🚀 Deploy Test Giveaway', ButtonStyle.Success),
        button(section.enabled !== false ? 'admin:giveaways:disable' : 'admin:giveaways:enable', section.enabled !== false ? '⏸️ Disable' : '▶️ Enable', ButtonStyle.Secondary),
        button('admin:giveaways:toggleMultiple', '🎟️ Multiple', ButtonStyle.Secondary),
        button('admin:giveaways:toggleRequireRole', '🔒 Role Req', ButtonStyle.Secondary),
        button('admin:giveaways:togglePing', '📣 Ping', ButtonStyle.Secondary)
      ),
...
function save(guild, updater) {
  return giveawaysStore.updateSection(guild.id, updater, guild);
}
