...
function makeTemplate(templateId, name, description, roleDefs, categoryDefs) {
  const roles = roleDefs.map(([roleName, color], index) => ({
    id: `template:${templateId}:role:${slugify(roleName)}`,
    name: roleName,
    color,
    hoist: index < 3,
    mentionable: false,
  ...
  for (const [categoryName, children] of categoryDefs) {
    ...
    for (const [channelName, type] of children) {
      channels.push(channelTemplate(`template:${templateId}:channel:${slugify(channelName)}`, channelName, type, categoryId, position++));
    }
  }
  ...
  return {
    meta: {
      ...
      createdBy: 'Goliath',
      updatedBy: 'Goliath',
      sourceGuildId: `template:${templateId}`,
    ...
    snapshot: {
      sourceGuild: { id: `template:${templateId}`, name },
...
function serializeChannel(channel) {
  return {
    ...
    availableTags: Array.isArray(channel.availableTags)
      ? channel.availableTags.map((tag) => ({ name: tag.name, moderated: Boolean(tag.moderated), emojiId: tag.emojiId || null, emojiName: tag.emojiName || null }))
      : [],
...
function snapshot(guild, selectedOptions = Object.keys(COPY_OPTIONS)) {
  ...
  const emojis = selected.has('emojis')
    ? guild.emojis.cache.map((emoji) => ({ id: emoji.id, name: emoji.name, animated: emoji.animated, url: emoji.url }))
    : [];
...
function templateChoices(guildId, selectedId = null) {
  ...
  return all.slice(0, 25).map((template) => ({
...
function copyPanel(interaction, session) {
  return {
    embeds: [embed('🛠️ Server Duplicator — Copy', [
      `**Source:** ${session.sourceGuildId ? guildById(interaction.client, session.sourceGuildId)?.name || session.sourceGuildId : '`Not selected`'}`,
      `**Destination:** ${session.destinationGuildId ? guildById(interaction.client, session.destinationGuildId)?.name || session.destinationGuildId : '`Not selected`'}`,
    ...
    components: [
      new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId(componentId(COPY_PREFIX, session.id, 'source')).setPlaceholder('Source server').addOptions(guildChoices(interaction.client, session.sourceGuildId))),
      new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId(componentId(COPY_PREFIX, session.id, 'destination')).setPlaceholder('Destination server').addOptions(guildChoices(interaction.client, session.destinationGuildId))),
...
function buildPanel(interaction, session) {
  ...
  return {
    ...
    components: [
      ...
      new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId(componentId(BUILD_PREFIX, session.id, 'destination')).setPlaceholder('Destination server').addOptions(guildChoices(interaction.client, session.destinationGuildId))),
      ...
      new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(componentId(BUILD_PREFIX, session.id, 'start')).setLabel(session.pendingConfirm && !session.dryRun ? 'Confirm Build' : session.dryRun ? 'Run Dry-Run' : 'Build Server').setStyle(session.pendingConfirm && !session.dryRun ? ButtonStyle.Danger : ButtonStyle.Success).setDisabled(!session.templateId || !session.destinationGuildId),
        new ButtonBuilder().setCustomId(componentId(BUILD_PREFIX, session.id, 'dryrun')).setLabel(session.dryRun ? 'Dry Run: ON' : 'Dry Run: OFF').setStyle(session.dryRun ? ButtonStyle.Primary : ButtonStyle.Secondary),
        new ButtonBuilder().setCustomId(componentId(BUILD_PREFIX, session.id, 'cancel')).setLabel('Cancel').setStyle(ButtonStyle.Danger)
      ),
    ],
    flags: MessageFlags.Ephemeral,
  };
...
async function applyEmojis(guild, snap, log, conflictMode) {
  ...
  for (const emoji of snap.emojis || []) {
    ...
    const name = names.has(emoji.name.toLowerCase()) && conflictMode === 'rename'
      ? uniqueName(names, emoji.name, 32).replace(/[^A-Za-z0-9_]/g, '_').slice(0, 32)
      : emoji.name;
