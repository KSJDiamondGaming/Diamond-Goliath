...
function pushError(log, stage, error) {
  ...
  console.error(`[DuplicatorV2] ${message}`, error);
}
...
async function clearDestination(guild, log) {
  for (const channel of [...guild.channels.cache.values()].sort((a, b) => b.position - a.position)) {
    try { await channel.delete('Goliath duplicator v2: replace destination'); log.deleted.channels += 1; } catch (error) { pushError(log, `Delete channel ${channel.name}`, error); }
  }
  ...
  for (const role of roles.values()) {
    try { await role.delete('Goliath duplicator v2: replace roles'); log.deleted.roles += 1; } catch (error) { pushError(log, `Delete role ${role.name}`, error); }
  }
}
...
async function applySettings(guild, snap, log) {
  ...
  await guild.edit(payload, 'Goliath duplicator v2: settings');
  ...
  for (const role of [...(snap.roles || [])].sort((a, b) => a.position - b.position)) {
    try {
      ...
      if (found && conflictMode === 'replace') {
        if (found.editable && found.position < botHighest) { await found.delete('Goliath duplicator v2: replace role'); log.deleted.roles += 1; }
      ...
      const created = await guild.roles.create({ name, colors: { primaryColor: Number(role.color || 0) }, hoist: Boolean(role.hoist), mentionable: Boolean(role.mentionable), permissions: safeRolePermissions(guild, role.permissions, role.name, log), reason: 'Goliath duplicator v2: role' });
...
function channelPayload(channel, parentId = null, name = null) {
  const payload = { name: name || channel.name, type: channel.type, reason: 'Goliath duplicator v2: channel' };
...
async function applyChannels(guild, snap, maps, log, conflictMode) {
  ...
  for (const category of categories) {
    try {
      ...
      if (found && conflictMode === 'replace' && found.deletable) { await found.delete('Goliath duplicator v2: replace category'); log.deleted.channels += 1; }
  ...
  for (const channel of channels) {
    try {
      ...
      if (found && conflictMode === 'replace' && found.deletable) { await found.delete('Goliath duplicator v2: replace channel'); log.deleted.channels += 1; }
...
async function applyPermissions(guild, snap, maps, log) {
  for (const sourceChannel of snap.channels || []) {
    try {
      ...
      await channel.permissionOverwrites.set(overwrites, 'Goliath duplicator v2: permissions');
...
async function applyEmojis(guild, snap, log, conflictMode) {
  ...
  for (const emoji of snap.emojis || []) {
    try {
      ...
      await guild.emojis.create({ attachment: emoji.url, name, reason: 'Goliath duplicator v2: emoji' });
...
async function executeStage(name, log, fn) {
  console.log(`[DuplicatorV2] Stage start: ${name}`);
  try { await fn(); console.log(`[DuplicatorV2] Stage complete: ${name}`); } catch (error) { pushError(log, name, error); }
}
