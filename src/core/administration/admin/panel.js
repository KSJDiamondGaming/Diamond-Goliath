...
function buildAdminPanel(guild, name = 'Unknown User', interaction = null) {
  const can = (key) => !interaction || hasGuildPermission(interaction, key); const fields = []; const actions = [];
  if (can('admin.automod.manage')) { fields.push({ name: '🛡️ Security & AutoMod', value: 'Protection rules and automated enforcement', inline: true }); actions.push(['admin:automod', '🛡️ Security', ButtonStyle.Primary]); }
  if (!interaction || hasAnyModulePermission(interaction)) { fields.push({ name: '🧩 Studios & Modules', value: 'Configure only the Studios and modules assigned to your roles', inline: true }); actions.push(['admin:modules', '🧩 Studios', ButtonStyle.Primary]); }
  if (can('admin.logs.manage')) { fields.push({ name: '📋 Logs & Audit', value: `${Object.values(LOG_TYPES).filter((value) => getLogChannelId(guild.id, value.key)).length}/5 log channels configured`, inline: true }); actions.push(['admin:logs', '📋 Logs', ButtonStyle.Primary]); }
  if (can('admin.backups.view')) { fields.push({ name: '🧱 Backup & Recovery', value: 'Server backup visibility and recovery requests', inline: true }); actions.push(['admin:backups', '🧱 Backups', ButtonStyle.Secondary]); }
  if (can('mod.panel.view')) { fields.push({ name: '🔐 Moderation Hub', value: 'Moderation cases, actions and tooling', inline: true }); actions.push(['admin:modpanel', '🔐 Moderation', ButtonStyle.Primary]); }
  if (can('admin.purge')) { fields.push({ name: '🧹 Server Utilities', value: 'Controlled server maintenance actions', inline: true }); actions.push(['admin:purge', '🧹 Purge', ButtonStyle.Danger]); }
  if (!interaction || canManageGuildAuthority(interaction)) { fields.unshift({ name: '👥 Staff & Permissions', value: 'Map guild roles to Goliath authority and control exact powers', inline: true }); actions.unshift(['admin:adminpanel', '👥 Permissions', ButtonStyle.Success]); }
  const embed = createEmbed('🛠️ Goliath Administration', 'Only controls you are authorized to use are shown.', name); if (fields.length) embed.addFields(fields); else embed.setDescription('No guild-manageable administration controls are currently assigned to your roles.'); return { embeds: [embed], components: buttonRows(actions) };
}
