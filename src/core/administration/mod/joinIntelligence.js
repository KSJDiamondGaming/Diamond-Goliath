...
function riskColor(score) {
  ...
  if (score >= 40) return 0xF0A202;
  if (score >= 20) return 0xFEE75C;
  return 0x57F287;
}

function buildEmbed(member, context, local, suspects, scanId) {
  ...
  const embed = new EmbedBuilder()
    .setColor(riskColor(Number(risk.score || 0)))
    .setTitle(`🧠 Join Intelligence • ${member.user.tag || member.user.username}`)
    .setDescription([
      `**Automatic join scan** • \`${scanId}\``,
      `**Member:** ${member.user} • \`${member.id}\``,
      '',
      'This is an automated intake snapshot using the same Goliath intelligence store that powers `/mod` Member Intelligence.',
    ].join('\n'))
    .addFields(
      {
        name: '👤 Account',
        value: [
          `Created: <t:${Math.floor((member.user.createdTimestamp || Date.now()) / 1000)}:F>${ageDays !== null ? ` • **${ageDays}d old**` : ''}`,
          `Joined: <t:${Math.floor((member.joinedTimestamp || Date.now()) / 1000)}:R>`,
          `Screening: **${member.pending ? 'Pending' : 'Complete'}** • Bot: **${member.user.bot ? 'Yes' : 'No'}**`,
        ].join('\n'),
        inline: false,
      },
      {
        name: '🚦 Risk',
        value: [
          `**${risk.score || 0}/100 • ${risk.label || '🟢 Low'}**`,
          `Watchlist: **${String(watch.state || 'clear').toUpperCase()}** • Local cases: **${local.caseCount}** • Active: **${local.activeCases}** • Warnings: **${local.warningCount}**`,
          riskFactors,
        ].join('\n').slice(0, 1024),
        inline: false,
      },
      {
        ...
        value: [
          `Observed guilds: **${history.length}** • Cross-guild cases: **${context.network?.caseCount || 0}** • Cross-guild bans: **${context.network?.banCount || 0}**`,
          `External records: **${externalCount}** • Verified: **${reputation.verifiedExternal || 0}** • Submitted: **${reputation.submitted || 0}** • Unverified: **${reputation.unverified || 0}**`,
        ].join('\n'),
        inline: false,
      },
      {
        name: '🔗 Account Correlation',
        value: `${suspectText.slice(0, 900)}\n\n*Heuristic correlation is an investigation aid, not proof of shared ownership.*`,
        inline: false,
      },
    )
    .setFooter({ text: 'Automatic Join Intelligence • Open /mod → Intelligence for the full interactive scan' })
    .setTimestamp();
...
async function scanMemberOnJoin(member) {
  ...
  recordModerationSystemEvent({
    ...
    after: {
      ...
      persistentLinkEvidence: [],
      visibleCapabilities: { automatic: true, fullSystemContext: true },
    },
    metadata: {
      targetId: member.id,
      automatic: true,
      trigger: 'guild_member_add',
      dataSources: ['discord_api', 'guild_cache', 'moderation_cases', 'warnings', 'member_intelligence', 'cross_guild_same_id_cases', 'watchlist', 'reputation'],
    },
  });

  if (!channel?.send) {
    console.warn(`[Join Intelligence] ${member.guild.name || member.guild.id}: enabled but no sendable output channel is configured.`);
    return { success: true, scanId, delivered: false, reason: 'missing_output_channel', risk: context.risk };
  }

  const embed = buildEmbed(member, context, local, suspects, scanId);
  await channel.send({ embeds: [embed], allowedMentions: { parse: [] } });
  return { success: true, scanId, delivered: true, channelId: channel.id, risk: context.risk, suspectedCount: suspects.length };
}
