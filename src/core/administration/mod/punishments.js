...
const ACTION_LABELS = Object.freeze({
  ...
  kick: 'Bulk Kick',
  ban: 'Bulk Ban',
  'remove-warning': 'Bulk Remove Warning',
  'remove-timeout': 'Bulk Remove Timeout',
});
const ACTION_EMOJIS = Object.freeze({ warn: '⚠️', timeout: '⏳', kick: '👢', ban: '🔨', 'remove-warning': '🗑️', 'remove-timeout': '✅' });
const BULK_PERMISSION_ACTIONS = Object.freeze({
  warn: 'bulk_warn',
  timeout: 'bulk_timeout',
  kick: 'bulk_kick',
  ban: 'bulk_ban',
  'remove-warning': 'bulk_remove_warning',
  'remove-timeout': 'bulk_remove_timeout',
});
...
function isValidTimeoutDuration(durationMs) { const value = Number(durationMs); return Number.isFinite(value) && value > 0 && value <= MAX_TIMEOUT_MS; }
function isValidDeleteDays(value) { const days = Number(value); return Number.isInteger(days) && days >= 0 && days <= 7; }
function parseDeleteDays(value) { const raw = String(value ?? '').trim(); if (!/^\d+$/.test(raw)) return null; const days = Number(raw); return isValidDeleteDays(days) ? days : null; }
function parseWarningExpiry(value) {
  ...
  return new Date(current.getTime() + amount * (match[2] === 'w' ? 7 * dayMs : dayMs)).toISOString();
}
...
function buildPunishmentModal(type, targetId) {
  ...
  const modal = new ModalBuilder().setCustomId(`mod_submit_${type}:${targetId}`).setTitle(config.title);
  const rows = [];
...
function bulkOperationHint(type) {
  if (type === 'warn') return 'warn or remove-warning';
  if (type === 'timeout') return 'timeout or remove-timeout';
  return type;
}
function buildBulkModal(type) {
  const modal = new ModalBuilder().setCustomId(`mod_submit_bulk_${type}`).setTitle(ACTION_LABELS[type] || 'Bulk Moderation');
  const rows = [
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('users').setLabel(type === 'warn' ? 'User IDs / warning case IDs (max 25)' : 'User IDs / mentions (max 25)').setStyle(TextInputStyle.Paragraph).setPlaceholder('IDs may be comma, space, mention, or newline separated').setRequired(true).setMaxLength(1000)),
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('operation').setLabel(`Operation: ${bulkOperationHint(type)}`.slice(0, 45)).setStyle(TextInputStyle.Short).setRequired(true).setMaxLength(20).setValue(type)),
  ];
  if (type === 'warn') {
    ...
    rows.push(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('duration').setLabel('Duration (10m, 1h, 1d)').setStyle(TextInputStyle.Short).setPlaceholder('1h').setRequired(false).setMaxLength(10)));
  } else if (type === 'ban') {
    rows.push(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('days').setLabel('Delete message days (0-7)').setStyle(TextInputStyle.Short).setPlaceholder('0').setRequired(true).setMaxLength(1)));
  }
...
function purgeExpiredPendingActions(guildId) { db.prepare('DELETE FROM pending_actions WHERE guild_id = ? AND expires_at <= ?').run(guildId, new Date().toISOString()); }
function createPendingAction(guildId, action = {}) {
  purgeExpiredPendingActions(guildId);
  const token = crypto.randomBytes(8).toString('hex');
  const createdAt = new Date().toISOString();
  const expiresAt = new Date(Date.now() + 600000).toISOString();
  db.prepare('INSERT INTO pending_actions (token, guild_id, moderator_id, target_id, type, payload, created_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(token, guildId, action.moderatorId || null, action.targetId || null, action.type || 'unknown', JSON.stringify(action.payload || {}), createdAt, expiresAt);
  return token;
}
function getPendingAction(guildId, token) {
  purgeExpiredPendingActions(guildId);
  const row = db.prepare('SELECT * FROM pending_actions WHERE guild_id = ? AND token = ?').get(guildId, token);
  if (!row) return null;
  return { token: row.token, moderatorId: row.moderator_id, targetId: row.target_id, type: row.type, payload: row.payload ? JSON.parse(row.payload) : {}, createdAt: row.created_at, expiresAt: row.expires_at };
}
function deletePendingAction(guildId, token) { db.prepare('DELETE FROM pending_actions WHERE guild_id = ? AND token = ?').run(guildId, token); }
function cleanError(error) { return String(error || '').replace(/^❌\s*/, ''); }
function normalizeDashboardContext(context = {}) { return { view: context.view || 'cases', actionFilter: context.actionFilter || 'all', statusFilter: context.statusFilter || 'all', page: Number(context.page) || 0 }; }
function buildConfirmCustomId(token, context = DEFAULT_DASHBOARD_CONTEXT) { const value = normalizeDashboardContext(context); return ['mod_confirm_action', token, value.view, value.actionFilter, value.statusFilter, value.page].join(':'); }
...
function createModerationCase(interaction, targetId, action, reason, metadata = {}, extras = {}) {
  return createCase({ guildId: interaction.guild.id, userId: targetId, moderatorId: interaction.user.id, action, reason, metadata, actorId: interaction.user.id, ...extras });
}
async function logAction(interaction, target, action, reason, caseId, metadata = {}) {
  ...
  return sendModLog({ guild: interaction.guild, target, moderator: interaction.user, action, reason, caseId, metadata });
}
function buildEngineOptions(interaction, action, reason, metadata = {}) {
  const config = ENGINE_ACTIONS[action];
  return { punishments: config.punishments, rule: config.rule, reason, moderator: interaction.user, source: 'moderation', ...(action === 'timeout' ? { durationMs: metadata.durationMs } : {}), ...(action === 'ban' ? { deleteDays: metadata.deleteDays } : {}) };
}
async function executeEnginePunishment(interaction, target, action, reason, metadata = {}, options = {}) {
  ...
  const report = await applyPunishmentEngine({ member: target, user: target.user, guild: interaction.guild }, buildEngineOptions(interaction, action, reason, metadata));
  if (!report.applied.includes(config.appliedKey)) throw new Error(`Failed to ${action} user. Failed: ${report.failedText}`);
  const caseMetadata = {
  ...
  const logMetadata = { ...caseMetadata, dmSent: report.dmSent };
  await logAction(interaction, target, options.logAction || config.logAction, reason, modCase.caseId, logMetadata);
...
async function submitTimeout(interaction, target) {
  ...
  if (!durationMs) { await safeReply(interaction, ephemeralError('Invalid duration. Use `10m`, `1h`, or `1d`.')); return { ok: false, target }; }
  if (!isValidTimeoutDuration(durationMs)) { await safeReply(interaction, ephemeralError('Timeout cannot exceed 28 days.')); return { ok: false, target }; }
  try {
...
async function submitPunishmentRequest(interaction, target, action, confirm = createConfirmation) {
  ...
  if (action === 'ban') {
    ...
    if (deleteDays === null) { await safeReply(interaction, ephemeralError('Delete message days must be 0-7.')); return { ok: false, target }; }
...
function readField(interaction, id, fallback = '') {
  try { return interaction.fields.getTextInputValue(id); } catch { return fallback; }
}
function parseBulkTokens(raw, actionType) {
  ...
  const invalid = [];
  const seen = new Set();
  const caseMode = actionType === 'remove-warning';
  for (const token of tokens) {
    const mention = token.match(/^<@!?(\d{16,20})>$/);
    const cleaned = mention ? mention[1] : token;
...
function parseBulkModalPayload(interaction, buttonAction) {
  ...
  const allowedOverrides = buttonAction === 'warn' ? new Set(['warn', 'remove-warning']) : buttonAction === 'timeout' ? new Set(['timeout', 'remove-timeout']) : new Set([buttonAction]);
  if (!allowedOverrides.has(operation)) return { error: `Operation must be ${[...allowedOverrides].join(' or ')}.` };
  const parsedTargets = parseBulkTokens(readField(interaction, 'users'), operation);
  const payload = {
    actionType: operation,
    ids: parsedTargets.ids,
    invalidTargets: parsedTargets.invalid,
    duplicateCount: parsedTargets.duplicateCount,
    reason: String(readField(interaction, 'reason')).trim(),
  };
  if (operation === 'warn') {
    payload.strikeWeight = parseStrikeWeight(readField(interaction, 'strike_weight', ''));
    payload.warnExpiryRaw = String(readField(interaction, 'warn_expiry', 'never') || 'never').trim().toLowerCase();
    payload.expiresAt = parseWarningExpiry(payload.warnExpiryRaw);
  }
  if (operation === 'timeout') payload.durationRaw = String(readField(interaction, 'duration')).trim();
  if (operation === 'ban') {
    payload.deleteDays = parseDeleteDays(readField(interaction, 'days'));
...
function validateBulkOptions(actionType, options = {}) {
  ...
  if (Array.isArray(options.ids) && options.ids.length > MAX_BULK_TARGETS) errors.push(`❌ Bulk moderation is limited to ${MAX_BULK_TARGETS} unique targets per batch.`);
  ...
  if (actionType === 'ban' && !isValidDeleteDays(options.deleteDays)) errors.push('❌ Delete message days must be between 0 and 7.');
  if (actionType === 'warn') {
    if (!options.strikeWeight) errors.push('❌ Strike weight must be a whole number from 1 to 5.');
    if (options.expiresAt === undefined) errors.push('❌ Warning expiry must be `7d`, `2w`, `1m`, or `never`.');
  }
  return errors;
}
function buildBulkPreview(options, batchId) {
  const ids = options.ids || [];
  const targetWord = options.actionType === 'remove-warning' ? 'warning case' : 'member';
  const lines = [
    `⚠️ **Confirm ${ACTION_LABELS[options.actionType] || 'Bulk Moderation'}**`,
    `Batch: \`${batchId}\``,
    `Targets: **${ids.length}** unique ${targetWord}${ids.length === 1 ? '' : 's'} (max ${MAX_BULK_TARGETS})`,
...
async function submitBulkModal(interaction, buttonAction) {
  ...
  const validationErrors = validateBulkOptions(parsed.payload.actionType, parsed.payload);
  if (validationErrors.length) return safeReply(interaction, { content: validationErrors.join('\n'), flags: 64 });
...
async function executePendingEngineAction(interaction, pending, target, action) {
  ...
  const metadata = action === 'ban' ? { deleteDays: Number(pending.payload.deleteDays || 0) } : {};
  const result = await executeEnginePunishment(interaction, target, action, reason, metadata);
  return { target, content: buildPendingSuccessContent(action, target, result.modCase, result.report) };
}
...
async function executeRemoveTimeout(interaction, pending, target) {
  ...
  if (reversedSourceCaseId) updateCaseStatus(interaction.guild.id, reversedSourceCaseId, 'reversed', interaction.user.id);
  const reason = reversedSourceCaseId ? `Removed timeout from case #${reversedSourceCaseId}` : 'Timeout removed from panel';
  const modCase = createModerationCase(interaction, target.id, 'remove-timeout', reason, {}, { relatedCaseId: reversedSourceCaseId, status: 'reversed' });
...
async function runBulkWarn(interaction, member, options) {
  ...
  const modCase = createModerationCase(interaction, member.id, 'warn', options.reason, {
    expiresAt: options.expiresAt || null,
    strikeWeight: options.strikeWeight,
    strikeSystem: 'weighted-v1',
    bulk: true,
    bulkBatchId: options.bulkBatchId,
    punishmentReport: report,
  });
  createWarning({ guildId: interaction.guild.id, userId: member.id, moderatorId: interaction.user.id, reason: options.reason, caseId: modCase.caseId, expiresAt: options.expiresAt || null });
  ...
  recordCaseAudit({
    ...
    metadata: { strikeWeight: options.strikeWeight, warningCount: warningContext.count, expiresAt: options.expiresAt || null, repeatCount: warningContext.repeatInfo.repeatCount, bulk: true, bulkBatchId: options.bulkBatchId },
  });
  ...
  await logAction(interaction, member, 'Bulk Warn', options.reason, modCase.caseId, {
    bulk: true,
    bulkBatchId: options.bulkBatchId,
    expiresAt: options.expiresAt || null,
    strikeWeight: options.strikeWeight,
...
async function runBulkRemoveWarning(interaction, caseIdRaw, options) {
  ...
  const removed = removeWarningByCaseId(interaction.guild.id, caseId, interaction.user.id);
  if (!removed) throw new Error('Warning is already absent or reversed.');
...
async function runBulkAction(interaction, options) {
  const validationErrors = validateBulkOptions(options.actionType, options);
  if (validationErrors.length) return safeReply(interaction, { content: validationErrors.join('\n'), flags: 64 });
  ...
  for (let index = 0; index < options.ids.length; index += 1) {
    ...
    try {
      ...
      failed.push(`❌ ${id} — ${String(error?.message || 'Failed to process.').slice(0, 180)}`);
    }
...
async function executePendingAction(discord, interaction, token, returnContext = {}) {
  ...
  if (!allowed) { deletePendingAction(interaction.guild.id, token); return true; }
  const target = await fetchTarget(interaction.guild, pending.targetId);
  const hierarchyError = checkHierarchy(interaction, target);
  if (hierarchyError && pending.type !== 'remove-warning') { deletePendingAction(interaction.guild.id, token); return safeReply(interaction, ephemeralError(cleanError(hierarchyError))); }
  const handlers = { ban: (valueInteraction, valuePending, valueTarget) => executePendingEngineAction(valueInteraction, valuePending, valueTarget, 'ban'), kick: (valueInteraction, valuePending, valueTarget) => executePendingEngineAction(valueInteraction, valuePending, valueTarget, 'kick'), 'remove-warning': executeRemoveWarning, 'remove-timeout': executeRemoveTimeout };
  const handler = handlers[pending.type];
  if (!handler) { deletePendingAction(interaction.guild.id, token); return safeReply(interaction, ephemeralError('Unknown pending action type.')); }
  try {
