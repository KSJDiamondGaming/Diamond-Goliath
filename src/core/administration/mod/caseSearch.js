    ...
    .addComponents(
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          ...
          .setPlaceholder('Optional — e.g. 123')
          .setRequired(false)
          .setMaxLength(12)
      ),
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          ...
          .setLabel('User ID')
          .setStyle(TextInputStyle.Short)
          .setPlaceholder('Optional Discord user ID')
          .setRequired(false)
          .setMaxLength(30)
      ),
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          .setCustomId('moderator_id')
          .setLabel('Moderator ID')
          .setStyle(TextInputStyle.Short)
          .setPlaceholder('Optional Discord moderator ID')
          .setRequired(false)
          .setMaxLength(30)
      ),
      new ActionRowBuilder().addComponents(
      ...
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          ...
          .setLabel('Status')
          .setStyle(TextInputStyle.Short)
          .setPlaceholder('active, reversed, expired')
          .setRequired(false)
          .setMaxLength(20)
      )
    );
...
function formatSearchCase(entry) {
  return [
    `**#${entry.caseId}** • ${entry.action} • ${entry.status || 'active'}`,
    `User: <@${entry.userId}> • Moderator: <@${entry.moderatorId}>`,
    `Reason: ${entry.reason || 'No reason provided'}`,
  ].join('\n');
}

function buildSearchResultsEmbed(result) {
  const description = result.results.length
    ? result.results.map(formatSearchCase).join('\n\n')
  ...
  return createEmbed({
    ...
    footer: `Results ${result.total ? `${result.page * result.pageSize + 1}-${Math.min((result.page + 1) * result.pageSize, result.total)} of ${result.total}` : '0'} • Page ${result.totalPages ? result.page + 1 : 0}/${result.totalPages || 0}`,
  });
}

function buildSearchButtons(result) {
  return [
    new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`mod_case_search_page:prev:${result.page}`)
        .setLabel('◀ Previous')
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(result.page <= 0),
      new ButtonBuilder()
        .setCustomId(`mod_case_search_page:next:${result.page}`)
        .setLabel('Next ▶')
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(result.page >= result.totalPages - 1 || result.totalPages === 0),
      new ButtonBuilder()
        .setCustomId('mod_case_search')
        .setLabel('New Search')
...
function buildSearchPayload(result) {
  return {
    embeds: [buildSearchResultsEmbed(result)],
    components: buildSearchButtons(result),
  };
}

async function openCaseSearch(interaction) {
  if (!canUseModAction(interaction.member, interaction.guild, 'view_case_detail')) {
    return safeReply(interaction, ephemeralError('No permission to search moderation cases.'));
  }
...
async function handleCaseSearchAction(interaction) {
  ...
  if (id === 'mod_case_search') return openCaseSearch(interaction);
  return false;
}
