const {
  ActionRowBuilder,
  ButtonBuilder,
  ButtonStyle,
  StringSelectMenuBuilder,
  EmbedBuilder
} = require('discord.js');

const { COLORS, EMOJIS } = require('../../utils/utility/uiConfig');
const { getStatusLabel } = require('../moderation/caseHelpers');

// =========================
// 🧭 Dashboard Navigation
// =========================
function buildDashboardNav(targetId, activeView = 'overview') {
  const items = [
    { view: 'overview', label: `${EMOJIS.DASHBOARD} Overview` },
    { view: 'actions', label: `${EMOJIS.ACTIONS} Actions` },
    { view: 'cases', label: `${EMOJIS.CASES} Cases` },
    { view: 'tools', label: `${EMOJIS.TOOLS} Tools` },
    { view: 'analytics', label: `${EMOJIS.ANALYTICS} Analytics` }
  ];

  return [
    new ActionRowBuilder().addComponents(
      items.map(item =>
        new ButtonBuilder()
          .setCustomId(`mod_dashboard:${targetId || 'none'}:${item.view}`)
          .setLabel(item.label)
          .setStyle(
            activeView === item.view
              ? ButtonStyle.Primary
              : ButtonStyle.Secondary
          )
      )
    )
  ];
}

// =========================
// 🛡️ Overview Embed
// =========================
function buildOverviewEmbed(guild, moderator, target, stats = {}) {
  return new EmbedBuilder()
    .setColor(COLORS.PRIMARY)
    .setTitle(`${EMOJIS.DASHBOARD} Moderation Dashboard`)
    .setDescription(
      target
        ? `${EMOJIS.USER} Managing **${target.user.tag}**`
        : `${EMOJIS.WARNING} No user selected yet.\nUse **${EMOJIS.USER} Select User** to begin.`
    )
    .addFields(
      {
        name: `${EMOJIS.MODERATOR} Moderator`,
        value: `${moderator}`,
        inline: true
      },
      {
        name: `${EMOJIS.WARNING} Warnings`,
        value: target ? String(stats.warningCount ?? 0) : '—',
        inline: true
      },
      {
        name: `${EMOJIS.CASES} Cases`,
        value: target ? String(stats.caseCount ?? 0) : '—',
        inline: true
      },
      {
        name: `${EMOJIS.CASE} Latest Case`,
        value: stats.lastCaseSummary || `${EMOJIS.ERROR} No cases found.`,
        inline: false
      }
    )
    .setTimestamp();
}

// =========================
// 📜 Cases Embed
// =========================
function buildCasesEmbed(target, cases, page, totalPages, actionFilter = 'all', statusFilter = 'all') {
  return new EmbedBuilder()
    .setColor(COLORS.PRIMARY)
    .setTitle(`${EMOJIS.CASES} Cases for ${target.user.tag}`)
    .setDescription(
      cases.length
        ? cases.map(entry =>
            `**#${entry.caseId}** • ${entry.action}\nStatus: ${getStatusLabel(entry)}\nReason: ${entry.reason || 'No reason'}`
          ).join('\n\n')
        : `${EMOJIS.ERROR} No cases found.`
    )
    .setFooter({
      text: `Action: ${actionFilter} | Status: ${statusFilter} | Page ${page + 1} of ${totalPages}`
    })
    .setTimestamp();
}

// =========================
// 📊 Analytics Embed
// =========================
function buildAnalyticsEmbed(guild, analytics) {
  return new EmbedBuilder()
    .setColor(COLORS.PRIMARY)
    .setTitle(`${EMOJIS.ANALYTICS} Moderation Analytics`)
    .setDescription(`${EMOJIS.FIRE} Stats for **${guild.name}**`)
    .addFields(
      {
        name: `${EMOJIS.CASE} Total Cases`,
        value: String(analytics.totalCases),
        inline: true
      },
      {
        name: `${EMOJIS.ACTIVE} Active`,
        value: String(analytics.activeCases),
        inline: true
      },
      {
        name: `${EMOJIS.EXPIRED} Expired`,
        value: String(analytics.expiredCases),
        inline: true
      },
      {
        name: `${EMOJIS.WARNING} Warns`,
        value: String(analytics.warnCount),
        inline: true
      },
      {
        name: `${EMOJIS.TIMEOUT} Timeouts`,
        value: String(analytics.timeoutCount),
        inline: true
      },
      {
        name: `${EMOJIS.BAN} Bans`,
        value: String(analytics.banCount),
        inline: true
      },
      {
        name: '🏆 Top Moderators',
        value: analytics.topModerators.length ? analytics.topModerators.join('\n') : 'None',
        inline: false
      },
      {
        name: '🎯 Top Moderated Users',
        value: analytics.topUsers.length ? analytics.topUsers.join('\n') : 'None',
        inline: false
      }
    )
    .setTimestamp();
}

// =========================
// ⚖️ Action Select Menu
// =========================
function buildActionSelect(targetId) {
  return [
    new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        .setCustomId(`mod_action_select:${targetId || 'none'}`)
        .setPlaceholder(`${EMOJIS.ACTIONS} Choose a moderation action`)
        .setDisabled(!targetId)
        .addOptions(
          { label: 'Warn', value: 'warn', emoji: EMOJIS.WARNING },
          { label: 'Timeout', value: 'timeout', emoji: EMOJIS.TIMEOUT },
          { label: 'Kick', value: 'kick', emoji: EMOJIS.KICK },
          { label: 'Ban', value: 'ban', emoji: EMOJIS.BAN },
          { label: 'Remove Warning', value: 'remove-warning', emoji: EMOJIS.DELETE },
          { label: 'Remove Timeout', value: 'remove-timeout', emoji: EMOJIS.SUCCESS }
        )
    )
  ];
}

// =========================
// 📄 Generic Pagination
// =========================
function buildPagination(targetId, page, totalPages) {
  return [
    new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`page_prev:${targetId}:${page - 1}`)
        .setLabel(`${EMOJIS.BACK} Prev`)
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(page <= 0),
      new ButtonBuilder()
        .setCustomId(`page_next:${targetId}:${page + 1}`)
        .setLabel(`Next ${EMOJIS.NEXT}`)
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(page >= totalPages - 1)
    )
  ];
}

module.exports = {
  buildDashboardNav,
  buildOverviewEmbed,
  buildCasesEmbed,
  buildAnalyticsEmbed,
  buildActionSelect,
  buildPagination
};