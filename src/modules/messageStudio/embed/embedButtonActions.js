...
function deploymentForMessage(guildId, messageId) {
  const deployments = Object.values(getAllEmbedDeployments(guildId) || {});
  return deployments.find((item) => String(item?.messageId || '') === String(messageId || '')) || null;
}
function presetForDeployment(guildId, deployment) {
...
function legacyActionFromId(customId) {
  const match = String(customId || '').match(/^embed-action:(.*):(\d+)$/);
  return match ? cleanAction(match[1]) : '';
}
function resolveButton(interaction) {
  const index = parseButtonIndex(interaction.customId);
...
function resolved(value, interaction) {
  try { return replaceVars(String(value || ''), interaction); }
...
function roleIsSafe(role, guild) {
  ...
  if (role.managed) return { ok: false, reason: 'That role is managed by Discord or another integration.' };
  const me = guild.members.me;
  if (!me || !role.editable || role.position >= me.roles.highest.position) return { ok: false, reason: 'Goliath cannot manage that role.' };
  if (DANGEROUS_ROLE_PERMISSIONS.some((permission) => role.permissions.has(permission))) {
    return { ok: false, reason: 'Self-service buttons cannot manage privileged moderation or administration roles.' };
  }
  return { ok: true };
}
async function roleAction(interaction, action, value) {
  ...
  if (action === 'add-role') {
    if (member.roles.cache.has(roleId)) return ephemeral(interaction, `ℹ️ You already have **${role.name}**.`);
    await member.roles.add(role, `Embed Studio button used by ${interaction.user.tag || interaction.user.id}`);
    return ephemeral(interaction, `✅ Added **${role.name}**.`);
  }
  if (action === 'remove-role') {
    if (!member.roles.cache.has(roleId)) return ephemeral(interaction, `ℹ️ You do not have **${role.name}**.`);
    await member.roles.remove(role, `Embed Studio button used by ${interaction.user.tag || interaction.user.id}`);
    return ephemeral(interaction, `✅ Removed **${role.name}**.`);
  }

  const hasRole = member.roles.cache.has(roleId);
  if (hasRole) await member.roles.remove(role, `Embed Studio role toggle used by ${interaction.user.tag || interaction.user.id}`);
  else await member.roles.add(role, `Embed Studio role toggle used by ${interaction.user.tag || interaction.user.id}`);
...
async function handleButtonAction(interaction) {
  ...
  const action = cleanAction(button?.action || legacyActionFromId(id));
  const value = button?.actionValue ?? button?.value ?? '';

  if (!action || action === 'custom' || action === 'none') {
    await ephemeral(interaction, 'ℹ️ This button does not have an action configured yet.');
    return true;
  }
  if (action === 'reply' || action === 'message') {
    const message = resolved(value || 'Button pressed.', interaction).slice(0, 2000);
    await ephemeral(interaction, message || 'Button pressed.');
    return true;
  }
  if (['toggle-role', 'add-role', 'remove-role'].includes(action)) {
    await roleAction(interaction, action, value);
    return true;
  }
  if (action === 'user-info') {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription([
        ...
        `**Roles:** ${member?.roles?.cache ? Math.max(0, member.roles.cache.size - 1) : 'Unknown'}`,
      ].join('\n'));
  ...
  if (action === 'server-info') {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription([
        ...
        `**Created:** ${guild?.createdTimestamp ? `<t:${Math.floor(guild.createdTimestamp / 1000)}:F>` : 'Unknown'}`,
      ].join('\n'));
...
module.exports = {
  ...
  parseButtonIndex,
  resolveButton,
  supportedActions: Object.freeze(['reply', 'toggle-role', 'add-role', 'remove-role', 'user-info', 'server-info']),
};
