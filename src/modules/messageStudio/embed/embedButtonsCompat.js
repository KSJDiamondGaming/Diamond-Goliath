...
function buttonsForPreset(preset) {
  ...
  if (selectedPanelIndex != null && Array.isArray(panels[selectedPanelIndex]?.buttons)) return panels[selectedPanelIndex].buttons;
...
function resolveButton(interaction) {
  ...
  if (!Number.isInteger(index) || index < 0 || index >= MAX_BUTTONS) return { index, button: null, deployment: null };
...
async function handleButtonAction(interaction) {
  ...
  if (action === 'user-info') {
    ...
    const embed = new EmbedBuilder().setColor(0x5865F2).setTitle('👤 Your Server Info').setDescription([
      `**User:** <@${interaction.user.id}>`, `**User ID:** \`${interaction.user.id}\``,
  ...
  if (action === 'server-info') {
    ...
    const embed = new EmbedBuilder().setColor(0x5865F2).setTitle(`🏠 ${guild?.name || 'Server'}`).setDescription([
      `**Members:** ${guild?.memberCount ?? 'Unknown'}`, `**Server ID:** \`${guild?.id || 'Unknown'}\``,
