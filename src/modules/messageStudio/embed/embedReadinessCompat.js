...
function usableUrl(value) {
  ...
  if (!raw) return true;
  if (hasVariable(raw)) return true;
...
function fieldText(panelData = {}) {
  return [panelData.title, panelData.description, panelData.authorName, panelData.authorIcon, panelData.authorUrl, panelData.footer, panelData.footerIcon, panelData.image, panelData.thumbnail,
    ...(Array.isArray(panelData.fields) ? panelData.fields.flatMap((field) => [field?.name, field?.value]) : [])].filter(Boolean).join('\n');
}
function unknownVariables(state) {
  const source = [
    ...(Array.isArray(state.panels) ? state.panels.map(fieldText) : []),
  ...
  const known = new Set((Array.isArray(panel.HELPERS) ? panel.HELPERS : []).map((item) => String(item).replace(/[{}]/g, '').toLowerCase()));
...
function getReadinessReport(interaction, state = panel.getSession(interaction)) {
  ...
  if (!state.channelId) push(errors, 'Choose a destination channel.');
  ...
  panels.forEach((item, index) => {
    ...
    fields.forEach((field, fieldIndex) => {
      if (!text(field?.name)) push(errors, `Panel ${number}, field ${fieldIndex + 1} is missing a name.`);
      if (!text(field?.value)) push(errors, `Panel ${number}, field ${fieldIndex + 1} is missing content.`);
    });
    [['Author icon', item?.authorIcon], ['Author URL', item?.authorUrl], ['Footer icon', item?.footerIcon], ['Thumbnail', item?.thumbnail], ['Image', item?.image]].forEach(([label, value]) => {
      if (text(value) && !usableUrl(value)) push(errors, `Panel ${number} ${label.toLowerCase()} is not a valid URL or variable.`);
    });
  ...
  buttons.forEach((button, index) => {
    const number = index + 1;
    if (!text(button?.label)) push(errors, `Button ${number} is missing a label.`);
    const url = text(button?.url);
    const action = text(button?.action).toLowerCase();
    ...
    if (ROLE_ACTIONS.has(action)) {
      const id = roleId(button?.actionValue);
      if (!id) push(errors, `Button ${number} role action has no valid role selected.`);
      else {
        const role = interaction.guild?.roles?.cache?.get?.(id);
        if (!role) push(errors, `Button ${number} selected role no longer exists.`);
        else if (role.id === interaction.guildId || role.managed) push(errors, `Button ${number} selected role cannot be managed by a self-service button.`);
        else if (!role.editable) push(errors, `Button ${number} selected role is above Goliath or otherwise not editable.`);
      }
    }
    const configuredRow = Number(button?.row);
    if (Number.isInteger(configuredRow) && configuredRow >= 1 && configuredRow <= MAX_BUTTON_ROWS) rowCounts[configuredRow - 1] += 1;
  });
  ...
  const unknown = unknownVariables(state);
  unknown.forEach((name) => push(warnings, `Variable \`{${name}}\` is not in the current helper list.`));
...
if (!panel.__readinessBuilderPatched && typeof panel.buildBuilderPanel === 'function') {
  ...
  panel.buildBuilderPanel = (interaction, ...args) => {
    const payload = originalBuildBuilderPanel(interaction, ...args);
    const rows = Array.isArray(payload?.components) ? payload.components : [];
    let target = rows.find((row, index) => index > 0 && Array.isArray(row?.components) && row.components.length < 5);
    if (!target && rows.length < 5) {
      target = new ActionRowBuilder();
      rows.push(target);
    }
    if (target && !target.components?.some?.((component) => component?.data?.custom_id === 'embed:readiness')) {
      target.addComponents(new ButtonBuilder().setCustomId('embed:readiness').setLabel('✅ Review').setStyle(ButtonStyle.Success));
    }
