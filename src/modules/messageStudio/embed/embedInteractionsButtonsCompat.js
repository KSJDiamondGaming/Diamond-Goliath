...
function saveButtons(i, state, buttons, selectedButtonIndex = state.selectedButtonIndex) {
  return panel.saveSession(i, { ...state, buttons, selectedButtonIndex, hasUnsavedChanges: true });
}
async function updateButtons(i) {
  await i.update(panel.buildButtonsManagerPanel(i));
  return true;
}
async function updateButtonOptions(i) {
  await i.update(panel.buildButtonOptionsPanel(i));
  return true;
}
async function replyButtons(i) {
  await i.reply({ ...panel.buildButtonsManagerPanel(i), flags: 64 });
  return true;
}
...
async function handleInteraction(i) {
  ...
  if (i.isButton?.()) {
    ...
    if (customId === 'embed:button-manager-add') {
      if (buttons.length >= panel.MAX_EMBED_BUTTONS) {
        await i.reply({ content: `Maximum of ${panel.MAX_EMBED_BUTTONS} buttons reached.`, flags: 64 });
        return true;
      }
      await i.showModal(panel.buttonEditorModal(state));
      return true;
    }
    if (customId === 'embed:button-manager-edit') {
      if (index == null) { await i.reply({ content: 'Select a button first.', flags: 64 }); return true; }
      await i.showModal(panel.buttonEditorModal(state, index));
      return true;
    }
    ...
    if (customId === 'embed:button-manager-remove') {
      ...
      const nextIndex = buttons.length ? Math.min(index, buttons.length - 1) : null;
      saveButtons(i, state, buttons, nextIndex);
  ...
  if (i.isModalSubmit?.() && (customId === 'embed:button-manager-save-new' || customId.startsWith('embed:button-manager-save:'))) {
    ...
    if (!label) {
      await i.reply({ content: 'A button label is required.', flags: 64 });
      return true;
    }
    if (!validUrlOrVariable(url)) {
      await i.reply({ content: 'Button links must be HTTP/HTTPS URLs or a URL-producing Embed Studio variable.', flags: 64 });
      return true;
    }
    if (url && action) {
      await i.reply({ content: 'Choose either a link URL or a custom action, not both.', flags: 64 });
      return true;
    }
    ...
    if (editingIndex == null) {
      ...
      buttons[editingIndex] = entry;
      selectedButtonIndex = editingIndex;
    }
