...
async function handleRoleSelectorInteraction(interaction) {
  ...
  try {
    ...
    if (id === 'admin:roleSelector:createDividerSubmit') { const name = interaction.fields.getTextInputValue('name').trim(); const role = await interaction.guild.roles.create({ name, reason: `Role Selector divider created by ${interaction.user.tag}` }); roleSelector.updateSection(interaction.guildId, (current) => ({ ...current, style: { ...current.style, anchorRoleId: role.id } }), { ...actor, action: 'role_selector_create_divider' }); return interaction.reply({ content: `✅ Created divider ${role}.`, flags: 64 }); }
    if (id === 'admin:roleSelector:anchor') { const roleId = interaction.values?.[0] || null; roleSelector.updateSection(interaction.guildId, (current) => ({ ...current, style: { ...current.style, anchorRoleId: roleId } }), { ...actor, action: 'role_selector_anchor' }); await roleSelector.syncManagedRoleAppearance(interaction.guild); return respond(interaction, buildStylePanel(interaction.guild)); }
    if (id === 'admin:roleSelector:togglePlacement') { roleSelector.updateSection(interaction.guildId, (current) => ({ ...current, style: { ...current.style, placement: current.style.placement === 'above' ? 'below' : 'above' } }), { ...actor, action: 'role_selector_placement' }); await roleSelector.syncManagedRoleAppearance(interaction.guild); return respond(interaction, buildStylePanel(interaction.guild)); }
    if (id === 'admin:roleSelector:toggleGrouped') { roleSelector.updateSection(interaction.guildId, (current) => ({ ...current, style: { ...current.style, keepGrouped: !current.style.keepGrouped } }), { ...actor, action: 'role_selector_grouping' }); await roleSelector.syncManagedRoleAppearance(interaction.guild); return respond(interaction, buildStylePanel(interaction.guild)); }
    if (id === 'admin:roleSelector:scanStyle') { roleSelector.updateSection(interaction.guildId, (current) => ({ ...current, style: { ...current.style, detectedFormat: roleSelector.detectGuildRoleFormat(interaction.guild) } }), { ...actor, action: 'role_selector_scan_style' }); return respond(interaction, await buildAdminPanel(interaction.guild, displayName(interaction))); }
    if (id === 'admin:roleSelector:applyStyle') { const section = roleSelector.getSection(interaction.guildId); if (!section.style.detectedFormat) throw new Error('Scan the guild first.'); roleSelector.updateSection(interaction.guildId, (current) => ({ ...current, style: { ...current.style, format: section.style.detectedFormat } }), { ...actor, action: 'role_selector_apply_style' }); await roleSelector.syncManagedRoleAppearance(interaction.guild); return respond(interaction, buildStylePanel(interaction.guild)); }
    if (id === 'admin:roleSelector:deploy') { await deploySelector(interaction); return respond(interaction, await buildAdminPanel(interaction.guild, displayName(interaction))); }
    if (id === 'admin:roleSelector:health') { const report = await healthService.buildHealth(interaction.guild); return respond(interaction, healthService.healthPayload(report)); }
    if (id === 'roleSelector:openGroup' && interaction.values?.[0] !== '__none__') return respond(interaction, memberGroupPayload(interaction.guild, interaction.member, interaction.values[0]));
    if (id.startsWith('roleSelector:choose:')) { const groupId = id.slice('roleSelector:choose:'.length); await roleSelector.applySelections(interaction.guild, interaction.member, groupId, interaction.values || [], { ...actor, action: 'role_selector_member_select' }); return respond(interaction, memberGroupPayload(interaction.guild, interaction.member, groupId)); }
    if (id.startsWith('roleSelector:clear:')) { const groupId = id.slice('roleSelector:clear:'.length); await roleSelector.clearSelection(interaction.guild, interaction.member, groupId, { ...actor, action: 'role_selector_member_clear' }); return respond(interaction, memberGroupPayload(interaction.guild, interaction.member, groupId)); }
    if (id === 'roleSelector:customHex') { await interaction.showModal(hexModal()); return true; }
    if (id === 'roleSelector:customHexSubmit') { const hex = interaction.fields.getTextInputValue('hex').trim(); const label = interaction.fields.getTextInputValue('label').trim(); await roleSelector.applyCustomColour(interaction.guild, interaction.member, hex, label, { ...actor, action: 'role_selector_custom_hex' }); return interaction.reply({ content: '✅ Your colour role has been updated.', flags: 64 }); }
    return false;
  } catch (error) {
