...
function normalizePalette(value) {
  ...
  for (const item of source) {
    ...
    const key = String(item?.key || item?.label || '').trim().toLowerCase().replace(/[^a-z0-9_-]/g, '-').slice(0, 40);
...
function updateSection(guildId, updater, meta = {}) {
  return normalizeSection(updateModuleSection(guildId, MODULE, (current) => {
    const normalized = normalizeSection(current);
    const next = typeof updater === 'function' ? updater(clone(normalized)) : { ...normalized, ...(updater || {}) };
    return { ...normalizeSection(next), updatedAt: now() };
  }, defaultSection(), meta));
}

function enabledPalette(guildId) { return getSection(guildId).palette.filter((item) => item.enabled).sort((a, b) => a.order - b.order); }
...
function suggestRoleStyle(guild) {
  ...
  for (const separator of candidates) {
    ...
    if (!best || count > best.count) best = { separator, count };
  }
  const emojiPattern = /^\p{Extended_Pictographic}/u;
  const emojiCount = names.filter((name) => emojiPattern.test(name)).length;
  const separator = best?.count >= 2 ? best.separator.trim() : '|';
  const icon = emojiCount >= Math.max(2, Math.floor(names.length * 0.2)) ? '🎨' : '';
  const format = icon ? `{icon} ${separator} {colour}` : `{colour}`;
  return { format, icon, separator, confidence: Math.min(1, (best?.count || 0) / Math.max(1, names.length)) };
}
...
function canManageRole(guild, role) {
  ...
  return Boolean(me && me.permissions.has(PermissionFlagsBits.ManageRoles) && role && !role.managed && role.position < me.roles.highest.position);
}

function sortManagedRecords(section) {
  return Object.values(section.managedRoles).sort((a, b) => {
    const ac = classifyHex(a.hex); const bc = classifyHex(b.hex);
    if (ac.familyOrder !== bc.familyOrder) return ac.familyOrder - bc.familyOrder;
    if (ac.hue !== bc.hue) return ac.hue - bc.hue;
    return ac.lightness - bc.lightness;
  });
}

async function reorderManagedRoles(guild) {
  const section = getSection(guild.id);
  if (!section.style.keepGrouped || !section.style.anchorRoleId) return { moved: 0, skipped: true };
  const anchor = guild.roles.cache.get(section.style.anchorRoleId) || await guild.roles.fetch(section.style.anchorRoleId).catch(() => null);
  if (!anchor) return { moved: 0, skipped: true, reason: 'anchor_missing' };
  const records = sortManagedRecords(section);
  const roles = records.map((record) => guild.roles.cache.get(record.roleId)).filter((role) => canManageRole(guild, role));
  if (!roles.length) return { moved: 0, skipped: true };
  let target = section.style.placement === 'above' ? anchor.position + 1 : Math.max(1, anchor.position - roles.length);
  let moved = 0;
  for (let index = roles.length - 1; index >= 0; index -= 1) {
    const role = roles[index];
    const desired = Math.max(1, target + index);
...
async function ensureManagedRole(guild, hex, label = null) {
  ...
  if (existingRecord) {
    const existing = guild.roles.cache.get(existingRecord.roleId) || await guild.roles.fetch(existingRecord.roleId).catch(() => null);
    if (existing) return existing;
  }
  const me = guild.members.me;
  if (!me?.permissions.has(PermissionFlagsBits.ManageRoles)) throw new Error('Goliath requires Manage Roles to create Colour Roles.');
  ...
  section = updateSection(guild.id, (current) => ({
    ...
    managedRoles: {
      ...
      [normalized]: { roleId: role.id, hex: normalized, label: resolvedLabel, family: classified.family, hue: classified.hue, createdAt: now(), unusedSince: null },
    },
...
function memberManagedRoleIds(member, section) {
  const managedIds = new Set(Object.values(section.managedRoles).map((record) => record.roleId));
...
async function chooseColour(member, hex, options = {}) {
  ...
  if (!options.force && !guildManager.isModuleEnabled(guild.id, MODULE)) throw new Error('Colour Roles is disabled.');
  let section = getSection(guild.id);
  const normalized = normalizeHex(hex);
  if (!normalized) throw new Error('Invalid HEX colour. Use #RRGGBB.');
  const paletteEntry = paletteEntryForHex(section, normalized);
  if (!paletteEntry && !section.customHexEnabled) throw new Error('Custom HEX colours are disabled for this server.');
  if (paletteEntry && !paletteEntry.enabled) throw new Error('That colour is disabled.');
  const role = await ensureManagedRole(guild, normalized, options.label || paletteEntry?.label || normalized);
  if (!canManageRole(guild, role)) throw new Error('Goliath cannot assign the selected colour role because of role hierarchy.');
  const previous = memberManagedRoleIds(member, section).filter((id) => id !== role.id);
  let removed = 0;
  for (const roleId of previous) {
    await member.roles.remove(roleId, 'Goliath Colour Roles switch').then(() => { removed += 1; });
  }
  if (!member.roles.cache.has(role.id)) await member.roles.add(role, 'Goliath Colour Roles selection');
  section = updateSection(guild.id, (current) => ({
    ...current,
    memberSelections: { ...current.memberSelections, [member.id]: { hex: normalized, roleId: role.id, selectedAt: now() } },
    managedRoles: { ...current.managedRoles, [normalized]: { ...current.managedRoles[normalized], unusedSince: null } },
    analytics: {
      ...current.analytics,
  ...
  return { roleId: role.id, hex: normalized, removed, label: section.managedRoles[normalized]?.label || paletteEntry?.label || normalized };
}
...
async function removeColour(member) {
  ...
  for (const roleId of roleIds) {
    await member.roles.remove(roleId, 'Goliath Colour Roles removal').then(() => { removed += 1; });
  }
  updateSection(member.guild.id, (current) => {
    const memberSelections = { ...current.memberSelections };
    delete memberSelections[member.id];
    return { ...current, memberSelections, analytics: { ...current.analytics, removals: Number(current.analytics.removals || 0) + removed } };
...
async function getUsage(guild) {
  ...
  for (const record of sortManagedRecords(section)) {
    ...
    rows.push({ ...record, roleName: role.name, count: members.length, members: members.map((member) => ({ id: member.id, name: member.displayName || member.user.username })) });
  }
  rows.sort((a, b) => b.count - a.count || a.label.localeCompare(b.label));
  const totalUsing = new Set(rows.flatMap((row) => row.members.map((member) => member.id))).size;
  return { rows, totalUsing, totalMembers: [...guild.members.cache.values()].filter((member) => !member.user?.bot).length };
}
...
async function markAndCleanupUnused(guild) {
  ...
  for (const [hex, record] of Object.entries(section.managedRoles)) {
    const role = guild.roles.cache.get(record.roleId) || await guild.roles.fetch(record.roleId).catch(() => null);
    if (!role) { delete nextManaged[hex]; continue; }
    if (role.members.size > 0) { nextManaged[hex] = { ...record, unusedSince: null }; continue; }
    const unusedSince = record.unusedSince ? new Date(record.unusedSince).getTime() : Date.now();
    nextManaged[hex] = { ...record, unusedSince: record.unusedSince || now() };
  ...
  section = updateSection(guild.id, (current) => ({
    ...current, managedRoles: nextManaged,
    analytics: { ...current.analytics, rolesDeleted: Number(current.analytics.rolesDeleted || 0) + deleted },
  }), { action: 'colour_roles_cleanup' });
  return { deleted, managed: Object.keys(section.managedRoles).length };
}

module.exports = {
  MODULE, DEFAULT_PALETTE, FAMILY_ORDER,
  defaultSection, normalizeSection, getSection, saveSection, updateSection, enabledPalette,
  cleanId, normalizeHex, hexToInt, rgbFromHex, rgbToHsl, classifyHex,
  suggestRoleStyle, roleNameFor, canManageRole, sortManagedRecords, reorderManagedRoles,
  ensureManagedRole, chooseColour, removeColour, getUsage, markAndCleanupUnused,
};
