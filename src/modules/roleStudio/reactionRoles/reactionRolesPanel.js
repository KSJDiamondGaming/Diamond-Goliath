...
async function buildSettingsPage(guild, memberDisplayName = 'Unknown User', notice = '') {
  ...
  return {
    ...
    components: [
      ...
      row(button('admin:reactionRoles', '⬅️ Back to Reaction Roles', ButtonStyle.Secondary)),
    ],
  };
}
...
function buildDisableConfirmation(guild) {
  ...
  return {
    ...
    components: [row(
      ...
      button('admin:reactionRoles:settings', 'Cancel', ButtonStyle.Secondary),
    )],
  };
}
...
function buildManagePicker(guild, notice = '') {
  const panels = reactionRoles.listPanels(guild.id);
  return {
    embeds: [new EmbedBuilder().setColor(notice ? 0x57f287 : 0x5865f2).setTitle('📚 Saved Reaction Role Panels').setDescription([
      noticeLine(notice), notice ? '' : null,
...
function buildWizard(guild, userId, showRemove = false, notice = '') {
  ...
  if (!existing) {
    ...
    components.push(row(button('admin:reactionRoles:source', '🔄 Change Source', ButtonStyle.Secondary)));
  }
  ...
  else components.push(row(modeSelect(draft.selectedMode)));
  components.push(
    row(
      ...
      button('admin:reactionRoles:wizard:deploy', draft.panelId ? '💾 Save' : '🚀 Deploy', ButtonStyle.Success, !ready),
  ...
  return {
    embeds: [new EmbedBuilder().setColor(notice || ready ? 0x57f287 : 0x5865f2)
      ...
      .setDescription([
        ...
        showRemove && draft.mappings.length ? '> Choose a mapping above to remove it.' : ready ? '> Ready to deploy.' : '> Complete the selections above and add at least one mapping.',
      ].filter((line) => line !== null).join('\n').slice(0, 4096))],
    components,
  };
}
...
function modal(customId, title, fields) {
  return new ModalBuilder().setCustomId(customId).setTitle(title).addComponents(...fields.map((field) => row(
    new TextInputBuilder().setCustomId(field.id).setLabel(field.label)
