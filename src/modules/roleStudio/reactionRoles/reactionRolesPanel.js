...
function settingsNavigationRow() {
  return row(
    ...
    button('admin:reactionRoles', '🏠 Reaction Roles', ButtonStyle.Secondary)
  );
}
...
function templateSelect(guildId, selectedId) {
  ...
  const menu = new StringSelectMenuBuilder()
    ...
    .setPlaceholder(
      templates.length
        ? 'Choose an Embed Studio template'
        : 'No Reaction Role templates available'
    )
    .setMinValues(1)
    .setMaxValues(1)
    .setDisabled(!templates.length);
  ...
  menu.addOptions(
    templates.length
      ...
      : [{
        label: 'Create a Reaction Role template in Embed Studio first',
        value: 'none',
      }]
  );
...
function modeSelect(mode) {
  return new StringSelectMenuBuilder()
    ...
    .addOptions([
      {
        label: 'Toggle role',
        description: 'Add on react and remove on unreact',
        value: reactionRoles.MODES.TOGGLE,
        default: mode === reactionRoles.MODES.TOGGLE,
      },
      {
        label: 'Add only',
        description: 'Add the role and keep it',
        value: reactionRoles.MODES.ADD,
        default: mode === reactionRoles.MODES.ADD,
      },
      {
        label: 'Remove only',
        description: 'Remove the role when reacted',
        value: reactionRoles.MODES.REMOVE,
        default: mode === reactionRoles.MODES.REMOVE,
      },
    ]);
}
...
async function buildReactionRolesAdminPanel(
  guild,
  ...
  const mappings = panels.reduce(
    (total, panel) => total + (panel.mappings?.length || 0),
    0
  );
  ...
  return {
    embeds: [
      new EmbedBuilder()
        ...
        .setDescription([
          ...
          `**Health:** ${health.healthy ? '✅ Healthy' : `⚠️ ${health.unhealthy || 0} panel(s) need attention`}`,
          '',
          'Module controls and maintenance tools are kept here.',
        ].filter((line) => line !== null).join('\n'))
        .setFooter({ text: `Requested by ${memberDisplayName}` })
        .setTimestamp(),
    ],
    components: [
      row(
        button(
          enabled
            ? 'admin:reactionRoles:disable:confirm'
            : 'admin:reactionRoles:enable',
          enabled ? '⏸️ Disable Module' : '▶️ Enable Module',
          enabled ? ButtonStyle.Danger : ButtonStyle.Success
        ),
        button(
          'admin:reactionRoles:repair',
          '🩺 Repair All',
          ButtonStyle.Secondary,
          !panels.length || !enabled
        )
      ),
...
function buildDisableConfirmation(guild) {
  ...
  return {
    ...
    components: [
      row(
        button(
          'admin:reactionRoles:disable:execute',
          'Disable Reaction Roles',
          ButtonStyle.Danger
        ),
        button(
          'admin:reactionRoles:settings',
          'Cancel',
          ButtonStyle.Secondary
        )
      ),
...
function buildDeleteConfirmation(guild, panelId) {
  ...
  return {
    embeds: [
      new EmbedBuilder()
        ...
        .setDescription([
          ...
          '• Remove Goliath’s reactions from the tracked message.',
          '',
          '**This will not:**',
          '• Delete the Discord message.',
          '• Remove roles members already have.',
          '',
    ...
    components: [
      row(
        button(
          `admin:reactionRoles:manage:remove:execute:${panelId}`,
          'Delete Panel',
          ButtonStyle.Danger
        ),
        button(
          `admin:reactionRoles:manage:view:${panelId}`,
          'Cancel',
          ButtonStyle.Secondary
        )
      ),
    ...
    embeds: [
      new EmbedBuilder()
        ...
        .setDescription([
          ...
          '',
          '**Create New Panel**',
          'Post a new message using a Reaction Role template from Embed Studio.',
          '',
          '**Use Existing Message**',
          'Attach emoji and role pairs to a message already in the server.',
        ].join('\n')),
    ],
    components: [
      row(
        button(
          'admin:reactionRoles:new:template',
          '✨ Create New Panel',
          ButtonStyle.Success
        ),
        button(
          'admin:reactionRoles:new:existing',
          '🔗 Use Existing Message',
          ButtonStyle.Primary
        ),
        button(
      ...
      row(
        button('admin:reactionRoles', '⬅️ Back', ButtonStyle.Secondary),
        button('admin:reactionRoles', '🏠 Home', ButtonStyle.Secondary),
        button(
          'admin:reactionRoles:manage',
          'Next ➡️',
          ButtonStyle.Secondary,
    ...
    embeds: [
      new EmbedBuilder()
        ...
        .setDescription([
          ...
          '',
          draft?.messageId
            ? `**Selected:** <#${draft.channelId}> · \`${draft.messageId}\``
            : '> No message selected.',
        ].filter((line) => line !== null).join('\n')),
    ],
    components: [
      row(
        button(
          'admin:reactionRoles:source:link',
          draft?.messageId ? '🔄 Change Message' : '🔗 Paste Message Link',
          ButtonStyle.Primary
        ),
        button(
          'admin:reactionRoles:source:continue',
      ...
      row(
        button(
          'admin:reactionRoles:wizard:cancel',
          '⬅️ Cancel Setup',
          ButtonStyle.Secondary
        ),
        button('admin:reactionRoles', '🏠 Home', ButtonStyle.Secondary)
...
function buildWizard(guild, userId, showRemove = false, notice = '') {
  ...
  components.push(
    row(
      button(
        'admin:reactionRoles:wizard:emoji',
        '➕ Add Role',
        ButtonStyle.Success,
        !draft.selectedRoleId
      ),
      button(
        'admin:reactionRoles:wizard:remove',
        '➖ Remove',
        ButtonStyle.Secondary,
  ...
  return {
    embeds: [
      new EmbedBuilder()
        ...
        .setTitle(
          draft.panelId
            ? '✏️ Edit Reaction Roles'
            : existing
              ? '🔗 Attach Reaction Roles'
        ...
        .setDescription([
          ...
          !existing
            ...
            : null,
          !existing
            ? `**Template:** ${draft.templateId ? `\`${draft.templateId}\`` : 'Not selected'}`
            : null,
          '',
          `### Mappings (${draft.mappings.length})`,
...
async function buildManagedPanel(guild, panelId, notice = '') {
  ...
  return {
    ...
    components: [
      row(
        button(
          `admin:reactionRoles:manage:edit:${panelId}`,
          '✏️ Edit Mappings',
          ButtonStyle.Primary
        ),
        button(
          `admin:reactionRoles:manage:repair:${panelId}`,
          '🔄 Sync & Repair',
          ButtonStyle.Secondary,
          panel.enabled === false
        ),
        button(
          `admin:reactionRoles:manage:${panel.enabled === false ? 'enable' : 'disable'}:${panelId}`,
          panel.enabled === false ? '▶️ Enable Panel' : '⏸️ Disable Panel',
          panel.enabled === false ? ButtonStyle.Success : ButtonStyle.Secondary
        ),
        button(
          `admin:reactionRoles:manage:remove:confirm:${panelId}`,
          '🗑️ Delete Panel',
          ButtonStyle.Danger
        )
      ),
      row(
        button('admin:reactionRoles:manage', '⬅️ Back', ButtonStyle.Secondary),
        button('admin:reactionRoles', '🏠 Home', ButtonStyle.Secondary),
        button('admin:reactionRoles:settings', '⚙️ Settings', ButtonStyle.Secondary)
      ),
    ],
...
async function deployDraft(guild, userId) {
  ...
  let panel;
  if (draft.panelId) {
    panel = await reactionRoles.updatePanelMappings(
      guild,
      draft.panelId,
      draft.mappings,
      userId
...
async function handleReactionRolesAdminInteraction(interaction) {
  ...
  try {
    if (
      ...
      && id === 'admin:reactionRoles:source:link:submit'
    ) {
      const parsed = reactionRoles.parseMessageReference(
        interaction.fields.getTextInputValue('messageLink')
      );
      if (parsed.guildId && parsed.guildId !== guild.id) {
        throw new Error('That message belongs to a different server.');
      }
      ...
      if (!message) {
        throw new Error('That message could not be found or Goliath cannot access it.');
      }

      reactionRoles.saveDraft(
        guild.id,
        userId,
    ...
    if (
      interaction.isModalSubmit?.()
      && id === 'admin:reactionRoles:wizard:emoji:submit'
    ) {
      const draft = reactionRoles.getDraft(guild.id, userId);
      if (!draft?.selectedRoleId) {
        throw new Error('Choose a role before adding an emoji.');
      }

      reactionRoles.addDraftMapping(
        guild.id,
        userId,
        {
      ...
      return interaction.reply({
        ...buildWizard(
          ...
          userId,
          false,
          '✅ Mapping added. Choose another role or deploy the panel.'
        ),
        ephemeral: true,
      });
    }

    if (id === 'admin:reactionRoles' || id === 'admin:reactionRoles:open') {
      return respond(
        interaction,
        await buildReactionRolesAdminPanel(guild, displayName(interaction))
      );
    }
    if (id === 'admin:reactionRoles:create') {
      return respond(interaction, buildCreatePicker(guild));
    }
    if (id === 'admin:reactionRoles:manage') {
      return respond(interaction, buildManagePicker(guild));
    }
    if (id === 'admin:reactionRoles:settings') {
      return respond(
        interaction,
        await buildSettingsPage(guild, displayName(interaction))
      );
    }
    if (id === 'admin:reactionRoles:disable:confirm') {
      return respond(interaction, buildDisableConfirmation(guild));
    }

    if (
      id === 'admin:reactionRoles:new:existing'
      || id === 'admin:reactionRoles:new:template'
    ) {
      const type = id.endsWith('template')
        ? reactionRoles.DRAFT_TYPES.TEMPLATE
        : reactionRoles.DRAFT_TYPES.EXISTING;
      ...
      return respond(
        ...
        type === reactionRoles.DRAFT_TYPES.EXISTING
          ? buildExistingMessageStep(guild, userId)
          : buildWizard(guild, userId)
      );
    }

    if (id === 'admin:reactionRoles:source') {
    ...
    if (
      ...
      && id === 'admin:reactionRoles:wizard:channel'
    ) {
      reactionRoles.saveDraft(
        guild.id,
        userId,
    ...
    if (
      ...
      && id === 'admin:reactionRoles:wizard:role'
    ) {
      reactionRoles.saveDraft(
        guild.id,
        userId,
    ...
    if (
      ...
      && id === 'admin:reactionRoles:wizard:mode'
    ) {
      reactionRoles.saveDraft(
        guild.id,
        userId,
    ...
    if (
      interaction.isStringSelectMenu?.()
      && id === 'admin:reactionRoles:wizard:template'
      && interaction.values[0] !== 'none'
    ) {
      reactionRoles.saveDraft(
        guild.id,
        userId,
        { templateId: interaction.values[0] },
        guild
      );
      return respond(interaction, buildWizard(guild, userId));
    }
    ...
    if (
      interaction.isStringSelectMenu?.()
      && id === 'admin:reactionRoles:wizard:remove:mapping'
    ) {
      reactionRoles.removeDraftMapping(
        guild.id,
        userId,
      ...
      return respond(
        interaction,
        buildWizard(guild, userId, false, '✅ Mapping removed.')
      );
    }

    if (
      interaction.isStringSelectMenu?.()
      && id === 'admin:reactionRoles:manage:panel'
      && interaction.values[0] !== 'none'
    ) {
      return respond(
        interaction,
        await buildManagedPanel(guild, interaction.values[0])
      );
    }
    ...
    if (id === 'admin:reactionRoles:wizard:remove') {
      return respond(interaction, buildWizard(guild, userId, true));
    }
    ...
    if (id === 'admin:reactionRoles:wizard:cancel') {
      ...
      return respond(
        interaction,
        await buildReactionRolesAdminPanel(
          guild,
          displayName(interaction),
          'ℹ️ Setup cancelled. No changes were deployed.'
        )
      );
    }
    ...
    if (id === 'admin:reactionRoles:wizard:deploy') {
      ...
      reactionRoles.clearDraft(guild.id, userId, guild);
      return interaction.editReply(await buildManagedPanel(
        guild,
        panel.panelId,
        wasEdit
          ? '✅ Changes saved and reactions synchronised.'
          : '✅ Panel deployed and reactions added.'
      ));
    }

    if (id.startsWith('admin:reactionRoles:manage:view:')) {
      return respond(
        interaction,
        await buildManagedPanel(guild, id.split(':').pop())
      );
    }

    if (id.startsWith('admin:reactionRoles:manage:remove:confirm:')) {
      return respond(
        interaction,
        buildDeleteConfirmation(guild, id.split(':').pop())
      );
    }
    ...
    if (id.startsWith('admin:reactionRoles:manage:edit:')) {
      ...
      if (!panel) {
        throw new Error('That reaction-role panel no longer exists.');
      }
    ...
    if (
      id.startsWith('admin:reactionRoles:manage:enable:')
      || id.startsWith('admin:reactionRoles:manage:disable:')
    ) {
      ...
      return interaction.editReply(await buildManagedPanel(
        ...
        panelId,
        enabling
          ? '✅ Panel enabled and reactions synchronised.'
          : '✅ Panel disabled. Its mappings remain saved.'
      ));
    }
    ...
    if (id.startsWith('admin:reactionRoles:manage:repair:')) {
      ...
      return interaction.editReply(await buildManagedPanel(
        guild,
        panelId,
        '✅ Panel checked and repaired successfully.'
      ));
    }
    ...
    if (id.startsWith('admin:reactionRoles:manage:remove:execute:')) {
      const panelId = id.split(':').pop();
      await interaction.deferUpdate();
      await reactionRoles.detachPanel(
        guild,
        panelId,
        { clearReactions: true }
      );
      return interaction.editReply(buildManagePicker(
        guild,
        '✅ Panel deleted. The Discord message was left in place.'
      ));
    }
    ...
    if (id === 'admin:reactionRoles:disable:execute') {
      ...
      return respond(
        ...
        await buildSettingsPage(
          guild,
          displayName(interaction),
          '✅ Reaction Roles disabled. Saved panels were preserved.'
        )
      );
    }
    ...
    if (id === 'admin:reactionRoles:repair') {
      ...
      return interaction.editReply(await buildSettingsPage(
        guild,
        displayName(interaction),
        notice
      ));
    ...
    const payload = {
      content: `❌ Reaction Roles failed: ${error.message}`,
      ephemeral: true,
    };
