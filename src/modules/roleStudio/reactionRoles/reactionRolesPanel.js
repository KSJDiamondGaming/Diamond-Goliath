...
function deploymentSelect(guildId) {
  ...
  menu.addOptions(panels.length ? panels.map((panel) => ({
    ...
    description: `${panel.enabled === false ? 'Disabled' : 'Enabled'} • ${panel.mappings.length} role${panel.mappings.length === 1 ? '' : 's'}`.slice(0, 100),
    value: panel.panelId,
  ...
  return {
    embeds: [new EmbedBuilder()
      ...
      .setDescription([
        ...
        '',
        '> Create a new panel or attach roles to a message already in your server.',
      ].join('\n'))
...
function buildManagePicker(guild) {
  return {
    embeds: [new EmbedBuilder()
      .setColor(0x5865f2)
      .setTitle('⚙️ Manage Reaction Roles')
      .setDescription('Choose a panel to view its mappings, status and management controls.')],
    components: [
...
function buildExistingMessageStep(guild, userId) {
  ...
  return {
    ...
    components: [
      ...
      row(
        button('admin:reactionRoles:source:continue', 'Continue', ButtonStyle.Success, !draft?.messageId),
...
function buildWizard(guild, userId, showRemove = false) {
  const draft = reactionRoles.getDraft(guild.id, userId);
  ...
  if (existing && !draft.messageId) return buildExistingMessageStep(guild, userId);
  ...
  if (!existing) {
    ...
    components.push(row(button('admin:reactionRoles:source', '🔄 Change Message', ButtonStyle.Secondary)));
  }

  components.push(
    row(new RoleSelectMenuBuilder()
      .setCustomId('admin:reactionRoles:wizard:role')
      .setPlaceholder(selectedRole ? `Selected: ${selectedRole.name}` : 'Choose a role')
      .setMinValues(1)
      .setMaxValues(1)),
  ...
  components.push(
    row(
      button('admin:reactionRoles:wizard:emoji', '➕ Add Emoji + Role', ButtonStyle.Success, !draft.selectedRoleId),
  ...
  return {
    embeds: [new EmbedBuilder()
      ...
      .setDescription([
        ...
        '',
        `**Next role:** ${selectedRole ? `<@&${selectedRole.id}>` : 'Choose a role above'}`,
...
async function buildManagedPanel(guild, panelId) {
  ...
  return {
    embeds: [new EmbedBuilder()
      .setColor(panel.enabled === false ? 0x747f8d : healthy ? 0x57f287 : 0xed4245)
      .setTitle(`🎭 ${panel.name}`)
      .setDescription([
        `**Status:** ${panel.enabled === false ? '⏸️ Disabled' : healthy ? '✅ Healthy' : '⚠️ Needs attention'}`,
        `**Channel:** <#${panel.channelId}>`,
        `**Message:** [Open message](https://discord.com/channels/${guild.id}/${panel.channelId}/${panel.messageId})`,
    ...
    components: [
      ...
      row(
        button(`admin:reactionRoles:manage:${panel.enabled === false ? 'enable' : 'disable'}:${panelId}`, panel.enabled === false ? '▶️ Enable' : '⏸️ Disable', panel.enabled === false ? ButtonStyle.Success : ButtonStyle.Secondary),
        button(`admin:reactionRoles:manage:remove:${panelId}`, '🗑️ Delete Panel', ButtonStyle.Danger),
      ),
...
function modal(customId, title, fields) {
  return new ModalBuilder()
    ...
    .setTitle(title)
    .addComponents(...fields.map((field) => row(new TextInputBuilder()
...
async function handleReactionRolesAdminInteraction(interaction) {
  ...
  try {
    if (interaction.isModalSubmit?.() && id === 'admin:reactionRoles:source:link:submit') {
      ...
      return interaction.reply({ ...buildExistingMessageStep(guild, userId), ephemeral: true });
    }
    ...
    if (id.startsWith('admin:reactionRoles:manage:remove:')) {
      ...
      return interaction.editReply(buildManagePicker(guild));
    }
    ...
    if (id === 'admin:reactionRoles:disable') {
