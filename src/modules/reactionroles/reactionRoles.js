...
function defaultSection() {
  return {
    ...
    panels: {},
    analytics: { attached: 0, assigned: 0, removed: 0, failed: 0, repaired: 0, lastActionAt: null },
    createdAt: now(),
    updatedAt: now(),
  };
}
...
function parseMessageReference(value, channelId = null) {
  ...
  if (!messageId || !parsedChannelId) throw new Error('Provide a Discord message link or both channelId and messageId.');
...
async function attachExistingMessage({ guild, messageReference, channelId, name, mappings = [], createdBy }) {
  if (!guild) throw new Error('Guild is required.');
  const message = await resolveMessage(guild, messageReference, channelId);
  const panel = savePanel(guild.id, { name, source: 'existing', channelId: message.channel.id, messageId: message.id, mappings, createdBy, status: 'attached' }, guild);
...
module.exports = {
  SECTION, MODES, getSection, setEnabled, listPanels, getPanel, findPanelByMessage, savePanel, removePanel,
  parseMessageReference, attachExistingMessage, updatePanelMappings, detachPanel, syncPanelReactions,
