...
function messagePoolPayload(interaction, type) {
  ...
  return {
    ...
    components: [
      row(button(`admin:birthdays:messages:${type}:edit`, '✏️ Edit Messages', ButtonStyle.Primary), button(`admin:birthdays:messages:${type}:defaults`, '♻️ Restore Defaults')),
...
function cardPayload(interaction) {
  ...
  return {
    ...
    components: [
      row(button('admin:birthdays:card:toggle', section.settings.useBirthdayEmbed ? '🟢 Card: On' : '⚪ Card: Off', section.settings.useBirthdayEmbed ? ButtonStyle.Success : ButtonStyle.Secondary), button('admin:birthdays:card:text', '✏️ Card Text', ButtonStyle.Primary), button('admin:birthdays:card:color', '🎨 Colour'), button('admin:birthdays:card:image', '🖼️ Image / GIF')),
      row(button('admin:birthdays:card:preview', '👁️ Preview'), button('admin:birthdays:card:defaults', '♻️ Restore Defaults')),
      row(button('admin:birthdays:celebration', '⬅️ Back')),
    ],
  };
}
...
function managementPayload(interaction) {
  ...
  const desc = [
    ...
    '', '**📅 Monthly Board**',
    `Channel: ${section.settings.monthlyBoardChannelId ? `<#${section.settings.monthlyBoardChannelId}>` : '**Not set**'}`,
    `Schedule: **1st monthly · ${section.settings.monthlyBoardTime} · ${section.settings.timezone}**`,
    `Leap day: **${section.settings.leapDayMode === 'mar1' ? '1 March' : '28 February'} in non-leap years**`,
    '', '**👥 Member Birthdays**',
    'Manage members’ stored birthday records.',
  ].join('\n');
  return {
    ...
    components: [
      row(new RoleSelectMenuBuilder().setCustomId('admin:birthdays:role').setPlaceholder('Birthday Role').setMinValues(0).setMaxValues(1).setDefaultRoles(section.settings.birthdayRoleId ? [section.settings.birthdayRoleId] : [])),
      ...
      row(button('admin:birthdays:manage', '👥 Manage Birthdays'), button('admin:birthdays:monthly:settings', '📅 Board Settings')),
      row(button('admin:birthdays', '⬅️ Back')),
    ],
  };
}
...
function toolsPayload() {
  return {
    ...
    components: [
      row(button('admin:birthdays:testmenu', '🧪 Test Centre'), button('admin:birthdays:health', '🩺 Health'), button('admin:birthdays:import', '📥 Import'), button('admin:birthdays:export', '📤 Export')),
      row(button('admin:birthdays', '⬅️ Back')),
    ],
  };
}
...
function customTimezoneModal(section) {
  return new ModalBuilder().setCustomId('admin:birthdays:timezone:custom:submit').setTitle('Custom Birthday Timezone').addComponents(row(new TextInputBuilder().setCustomId('timezone').setLabel('IANA timezone').setStyle(TextInputStyle.Short).setRequired(true).setValue(section.settings.timezone).setPlaceholder('Europe/London')));
}
function messagesModal(section, type) {
  ...
  return new ModalBuilder().setCustomId(`admin:birthdays:messages:${type}:submit`).setTitle(group ? 'Group Birthday Messages' : 'Individual Birthday Messages').addComponents(row(new TextInputBuilder().setCustomId('messages').setLabel('One rotating message per line').setStyle(TextInputStyle.Paragraph).setRequired(true).setMaxLength(4000).setValue(values.join('\n'))));
}
...
function cardColorModal(section) {
  return new ModalBuilder().setCustomId('admin:birthdays:card:color:submit').setTitle('Birthday Card Colour').addComponents(row(new TextInputBuilder().setCustomId('color').setLabel('Embed colour hex').setStyle(TextInputStyle.Short).setRequired(true).setValue(section.settings.cardColor).setPlaceholder('#5865F2')));
}
...
function importModal() {
  return new ModalBuilder().setCustomId('admin:birthdays:import:submit').setTitle('Import Birthdays JSON').addComponents(row(new TextInputBuilder().setCustomId('json').setLabel('Paste Goliath birthday JSON').setStyle(TextInputStyle.Paragraph).setRequired(true).setMaxLength(4000)));
}
