...
function pollPayload(poll) {
  return {
    embeds: [polls.buildPollEmbed(poll)],
...
async function deployPoll(guild, pollId, channelId, meta = {}) {
  ...
  poll.status = 'active';
  poll.closedAt = null;
  poll.updatedAt = polls.now();

  if (existing?.edit) {
    await existing.edit(pollPayload(poll));
    section.polls[poll.id] = poll;
    return {
      section: polls.saveSection(guild.id, section, meta),
      poll,
  ...
  try {
    const saved = polls.saveSection(guild.id, section, meta);
...
async function processVote(interaction, pollId, optionId) {
  ...
  if (!option) { await respond(interaction, 'That poll option no longer exists.'); return true; }
  ...
  if (!poll.allowMultipleVotes && !alreadyVoted) {
    for (const candidate of poll.options) {
      ...
      candidate.votes = candidate.votes.filter((id) => id !== userId);
  ...
  poll.updatedAt = polls.now();
  section.polls[poll.id] = poll;
  polls.saveSection(guildId, section, { actorId: userId });
...
async function vote(interaction) {
  ...
  return queueVote(`${interaction.guildId || 'unknown'}:${pollId}`, () => processVote(interaction, pollId, optionId));
}
async function buildHealth(guild) {
  ...
  for (const poll of activePolls) {
    ...
    if (!await resolvePollMessage(guild, poll)) {
      issues.push({ code: 'poll_message_missing', severity: 'error', pollId: poll.id, channelId: poll.channelId, messageId: poll.messageId });
    }
  }
  return {
...
function exportConfig(guildId) {
  return { module: 'polls', exportedAt: polls.now(), guildId: String(guildId), config: polls.getSection(guildId) };
}
...
async function closeExpiredPollsForGuild(guild) {
  ...
  for (const poll of Object.values(section.polls || {})) {
    ...
    try {
      ...
      if (deployedAt && Date.now() - deployedAt >= maxAgeMs) {
