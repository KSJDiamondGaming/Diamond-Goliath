...
async function createGiveaway(guildOrChannel, input = {}) {
  ...
  if (!giveawaysStore.isEnabled(guild.id)) throw new Error('Giveaways are disabled.');
...
async function endGiveawayById(client, guildId, giveawayId, actorMember = null) {
  ...
  if (!giveawaysStore.isEnabled(guildId)) throw new Error('Giveaways are disabled.');
...
async function checkExpiredGiveaways(client) {
  ...
  for (const guild of client?.guilds?.cache?.values?.() || []) {
    if (!giveawaysStore.isEnabled(guild.id)) continue;
