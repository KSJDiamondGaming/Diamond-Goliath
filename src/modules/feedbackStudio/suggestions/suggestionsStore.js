...
function normalizeSection(section = {}) {
  ...
  return {
    ...
    suggestions: Object.fromEntries(
      Object.entries(suggestions).map(([id, suggestion]) => {
...
function getSuggestion(guildId, suggestionId) {
  return getSection(guildId).suggestions?.[cleanString(suggestionId, '', 80)] || null;
}
...
function updateSuggestion(guildId, suggestionId, updater, guildOrMeta = {}) {
  return updateSection(guildId, (section) => {
    ...
    return {
      ...
      suggestions: {
        ...(section.suggestions || {}),
        [suggestionId]: normalizeSuggestion({ ...current, ...next, suggestionId, updatedAt: now() }),
      },
      updatedAt: now(),
    };
