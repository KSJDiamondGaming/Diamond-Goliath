...
function getSession(interaction) {
  return sessions.get(sessionKey(interaction)) || { routeType: 'default', creatorId: null, creatorPage: 0, platform: 'youtube', view: 'hub' };
}
...
function platformSelect(selected) {
  return row(new StringSelectMenuBuilder().setCustomId(`${P}channel:platform:select`).setPlaceholder('Choose a platform').setMinValues(1).setMaxValues(1).addOptions(PLATFORMS.map((platform) => ({ label: `${PLATFORM_EMOJI[platform]} ${PLATFORM_LABEL[platform]}`, value: platform, description: `Route all ${PLATFORM_LABEL[platform]} content to one channel.`, default: platform === selected }))));
}
...
function syncCreatorOverride(config, creator) { if (!creator) return config; for (const accountId of creator.accountIds || []) { const account = config.accounts?.[accountId]; if (!account) continue; if (creator.alertChannelId) applyCreatorOverrideToAccount(account, creator.alertChannelId); else clearCreatorOverrideFromAccount(account); } return config; }
...
function channelsHubPayload(interaction) {
  ...
  const creatorCount = sortedCreators(config).filter((creator) => creator.alertChannelId).length;
  ...
  const description = [
    ...
    '**Routing Priority**', '1. Creator/User override', '2. Platform override', '3. Dedicated content-type channel', '4. Default channel',
  ].join('\n');
...
function creatorChannelsPayload(interaction) {
  ...
  const overridden = sortedCreators(config).filter((creator) => creator.alertChannelId); const summary = overridden.length ? overridden.slice(0, 12).map((creator) => `• **${creator.displayName || creator.creatorId}** → <#${creator.alertChannelId}>`).join('\n') + (overridden.length > 12 ? `\n• …and ${overridden.length - 12} more` : '') : 'No creator-specific channels are configured.';
  const description = ['Send every automatic Social Studio post for a selected creator to one Discord channel.', '', '**How it works**', 'The override applies to all social accounts linked to that Creator Profile.', 'If a creator has no override, their posts continue through Platform Overrides, Dedicated Content Channels, then the Default Channel.', '', '**Current Overrides**', summary, '', selected ? `**Selected Creator:** ${selected.displayName || selected.creatorId}` : '**Selected Creator:** None', selected ? `**Automatic Post Channel:** ${selected.alertChannelId ? `<#${selected.alertChannelId}>` : 'Uses server routing'}` : 'Choose a creator below.'].join('\n');
  const components = []; if (creatorMenu.row) components.push(creatorMenu.row); if (selected) { components.push(channelSelect(`${P}channel:creator:route`, selected.alertChannelId || null, `Choose ${String(selected.displayName || 'creator').slice(0, 70)}'s automatic post channel`)); components.push(row(button(`${P}channel:creator:clear`, '↩️ Use Server Routing', ButtonStyle.Secondary, !selected.alertChannelId))); }
  ...
  components.push(row(button(`${P}channels`, '⬅️ Channels'), button(`${P}main`, '🏠 Social Studio'))); return { embeds: [embed(config, '👤 Creator Channel Overrides', description, interaction)], components };
}
...
async function handle(interaction) {
  ...
  if (id === `${P}channel:creator:open`) { setSession(interaction, { view: 'creator', creatorId: null, creatorPage: 0 }); return update(interaction, creatorChannelsPayload(interaction)); }
  if (id === `${P}channel:creator:select`) { setSession(interaction, { view: 'creator', creatorId: interaction.values?.[0] || null }); return update(interaction, creatorChannelsPayload(interaction)); }
  if (id === `${P}channel:creator:prev` || id === `${P}channel:creator:next`) { const state = getSession(interaction); setSession(interaction, { creatorPage: Math.max(0, state.creatorPage + (id.endsWith('next') ? 1 : -1)), creatorId: null, view: 'creator' }); return update(interaction, creatorChannelsPayload(interaction)); }
...
function installStoreCompatibility() {
  ...
  store.upsertCreatorAccount = function upsertCreatorAccountWithRoute(guildId, creatorId, account, duplicateIds = [], meta = {}) { const creator = store.getCreator(guildId, creatorId); let nextAccount = account; if (creator?.alertChannelId) nextAccount = applyCreatorOverrideToAccount({ ...account, alertChannels: cloneObject(account?.alertChannels) }, creator.alertChannelId); return originalUpsertCreatorAccount(guildId, creatorId, nextAccount, duplicateIds, meta); };
