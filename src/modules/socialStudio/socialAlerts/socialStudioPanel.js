...
async function handleInteraction(i) {
  ...
  if (id === `${P}creator:profile`) { const cid = getCreatorSession(i).creatorId, creator = config.creators[cid]; if (!creator) throw new Error('Select a creator profile first.'); setAccountSession(i, { creatorId: cid, accountId: null, platforms: [], routeType: 'default' }); return respond(i, buildProfileManagePanel(i, config, creator)); }
  ...
  if (id === `${P}creator:profile:toggle`) { const creator = config.creators[getCreatorSession(i).creatorId]; if (!creator) throw new Error('The selected creator profile no longer exists.'); creator.enabled = creator.enabled === false; creator.updatedAt = now(); saveConfig(i.guildId, config, i.guild, actorId); return respond(i, buildProfileManagePanel(i, getConfig(i.guildId), getConfig(i.guildId).creators[creator.creatorId])); }
  if (id === `${P}creator:clear`) { setCreatorSession(i, { creatorId: null }); accountSessions.delete(sessionKey(i)); return respond(i, buildSectionPanel(i, 'creators')); }
  if (id === `${P}creator:delete`) { const s = getAccountSession(i), creatorId = s.creatorId || getCreatorSession(i).creatorId, c = config.creators[creatorId]; if (!c) throw new Error('The selected creator profile no longer exists.'); setCreatorSession(i, { creatorId }); setAccountSession(i, { creatorId }); return respond(i, { embeds: [embed(config, '⚠️ Delete Creator Profile', `Delete **${c.displayName}**? Linked accounts remain stored but unassigned.`, who(i))], components: [row(btn(`${P}creator:delete:cancel`, '⬅️ Back'), btn(`${P}creator:delete:confirm`, '🗑️ Delete Profile', ButtonStyle.Danger))] }); }
  if (id === `${P}creator:delete:cancel`) { const c = config.creators[getCreatorSession(i).creatorId]; return c ? respond(i, buildProfileManagePanel(i, config, c)) : respond(i, buildSectionPanel(i, 'creators')); }
  ...
  if (id === `${P}account:platforms`) { const platforms = (i.values || []).filter((p) => PLATFORMS.includes(p)).slice(0, 5), s = setAccountSession(i, { platforms }); const creator = config.creators[s.creatorId]; return s.mode === 'add' && creator ? respond(i, buildAccountAddPanel(i, config, creator)) : respond(i, buildSectionPanel(i, 'accounts')); }
  if (id === `${P}account:reset`) { setAccountSession(i, { accountId: null, platforms: [], routeType: 'default' }); return respond(i, buildSectionPanel(i, 'accounts')); }
  ...
  if (id === `${P}account:delete`) { const a = config.accounts[getAccountSession(i).accountId]; if (!a) throw new Error('The selected account no longer exists.'); return respond(i, { embeds: [embed(config, '⚠️ Delete Social Account', `Delete **${LABEL[a.platform]} · ${a.username || a.externalId}**?`, who(i))], components: [row(btn(`${P}account:delete:cancel`, '⬅️ Back'), btn(`${P}account:delete:confirm`, '🗑️ Delete Account', ButtonStyle.Danger))] }); }
  if (id === `${P}account:delete:cancel`) return respond(i, buildSectionPanel(i, 'accounts'));
