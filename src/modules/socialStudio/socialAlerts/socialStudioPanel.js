...
function accountSingleModal() {
  return new ModalBuilder().setCustomId(`${P}account:create-one`).setTitle('Add Social Account').addComponents(
    row(new TextInputBuilder().setCustomId('platform').setLabel('Platform').setPlaceholder('Example: Twitch, YouTube, TikTok, Kick').setStyle(TextInputStyle.Short).setMaxLength(40).setRequired(true)),
    row(new TextInputBuilder().setCustomId('accountValue').setLabel('Username, channel ID or URL').setPlaceholder('Paste the profile URL, username or channel ID here').setStyle(TextInputStyle.Short).setMaxLength(500).setRequired(true)),
  );
}
...
function buildMainPanel(guild, requestedBy = 'Unknown User') {
  ...
  return { embeds: [embed(c, '📣 Social Studio', d, requestedBy)], components: [row(btn(`${P}creators`, '👥 Creator Profiles', ButtonStyle.Primary), btn(`${P}channels`, '📂 Channels'), btn(`${P}templates`, '🎨 Templates')), navigation('main')] };
}
...
function buildSectionPanel(i, name) {
  ...
  if (name === 'accounts') {
    ...
    const d = creator ? [`👤 **${creator.displayName}**`, '', '**Profile**', `Status: ${creator.enabled === false ? '⏸️ Paused' : '🟢 Monitoring'}`, `Group / Team: ${creator.group || 'Not set'}`, `Tags: ${creator.tags?.length ? creator.tags.join(', ') : 'None'}`, '', '**Accounts**', `Linked: ${linked.length}`, `Selected: ${active ? `${LABEL[active.platform]} — ${active.username || active.externalId}` : linked.length ? 'Choose an account below.' : 'None yet.'}`, ...(list ? ['', list] : ['', 'No linked social accounts.'])].join('\n') : `Select a creator profile to manage profile details and linked social accounts.\n\n**Profiles:** ${creators.length}`;
    const components = []; if (creators.length) { components.push(creatorSelect(creators, session.creatorId, `${P}account:creator`, 'Select a creator profile to manage')); if (linked.length) components.push(accountSelect(linked, getAccountSession(i).accountId)); components.push(row(btn(`${P}creator:edit`, '📝 Edit Profile', ButtonStyle.Secondary, !creator), btn(`${P}account:edit`, '🛠️ Manage Account', ButtonStyle.Primary, !active))); components.push(row(btn(`${P}account:creator:toggle`, creator?.enabled === false ? '▶️ Resume Profile' : '⏸️ Pause Profile', creator?.enabled === false ? ButtonStyle.Success : ButtonStyle.Secondary, !creator), btn(`${P}creator:delete`, '🗑️ Delete', ButtonStyle.Danger, !creator), btn(`${P}account:reset`, '🔄 Clear Selection'), btn(`${P}creators`, '⬅️ Creator Profiles'), btn(`${P}settings`, '⚙️ Settings'))); } else components.push(row(btn(`${P}creators`, '👥 Create Creator Profile', ButtonStyle.Primary))); return { embeds: [embed(config, '🛠️ Manage Creator', d, who(i), active ? platformColor(active.platform) : creator ? creatorAccent(linked) : null)], components };
  }
...
function opensModal(id) { return id === `${P}creator:new` || id === `${P}creator:edit` || id === `${P}creator:change` || id === `${P}account:new` || id === `${P}account:continue` || id === `${P}account:change` || id === `${P}account:move:new` || id === `${P}automation:quiet` || (id.startsWith(`${P}template:`) && !id.startsWith(`${P}template:save:`)); }
async function handleInteraction(i) {
  ...
  if (id === `${P}account:new`) { const cid = getCreatorSession(i).creatorId; if (!cid || !config.creators[cid]) throw new Error('Select a creator profile first.'); setAccountSession(i, { creatorId: cid, accountId: null, platforms: [], routeType: 'default' }); await i.showModal(accountSingleModal()); return true; }
  ...
  if (id === `${P}creator:edit`) { const s = getAccountSession(i), creatorId = s.creatorId || getCreatorSession(i).creatorId, c = config.creators[creatorId]; if (!c) throw new Error('Select a creator profile first.'); setCreatorSession(i, { creatorId }); setAccountSession(i, { creatorId }); await i.showModal(creatorModal(c)); return true; }
  ...
  if (id === `${P}creator:toggle`) { const creator = config.creators[getCreatorSession(i).creatorId]; if (!creator) throw new Error('The selected creator profile no longer exists.'); creator.enabled = creator.enabled === false; creator.updatedAt = now(); saveConfig(i.guildId, config, i.guild, actorId); return respond(i, buildSectionPanel(i, 'accounts')); }
  if (id === `${P}creator:delete`) { const s = getAccountSession(i), creatorId = s.creatorId || getCreatorSession(i).creatorId, c = config.creators[creatorId]; if (!c) throw new Error('The selected creator profile no longer exists.'); setCreatorSession(i, { creatorId }); setAccountSession(i, { creatorId }); return respond(i, { embeds: [embed(config, '⚠️ Delete Creator Profile', `Delete **${c.displayName}**? Linked accounts remain stored but unassigned.`, who(i))], components: [row(btn(`${P}creator:delete:cancel`, '⬅️ Back to Manage'), btn(`${P}creator:delete:confirm`, '🗑️ Delete Profile', ButtonStyle.Danger))] }); }
  if (id === `${P}creator:delete:cancel`) return respond(i, buildSectionPanel(i, 'accounts'));
  ...
  if (id === `${P}account:creator:toggle`) { const s = getAccountSession(i), creator = config.creators[s.creatorId]; if (!creator) throw new Error('Select a creator profile first.'); creator.enabled = creator.enabled === false; creator.updatedAt = now(); saveConfig(i.guildId, config, i.guild, actorId); setCreatorSession(i, { creatorId: creator.creatorId }); return respond(i, buildSectionPanel(i, 'accounts')); }
  ...
  if (id === `${P}account:create-one`) { const s = getAccountSession(i), c = config.creators[s.creatorId]; if (!c) throw new Error('Select a creator profile first.'); const rawPlatform = i.fields.getTextInputValue('platform').trim().toLowerCase().replace(/[^a-z]/g, ''); const platformAliases = { yt: 'youtube', youtube: 'youtube', twitch: 'twitch', tiktok: 'tiktok', tik: 'tiktok', kick: 'kick', facebook: 'facebook', fb: 'facebook', instagram: 'instagram', insta: 'instagram', x: 'x', twitter: 'x' }; const platform = platformAliases[rawPlatform]; if (!PLATFORMS.includes(platform)) throw new Error(`Platform must be one of: ${PLATFORMS.map((p) => LABEL[p]).join(', ')}.`); const raw = i.fields.getTextInputValue('accountValue').trim(); if (!raw) throw new Error('Account username, channel ID or URL is required.'); const r = upsertAccount(config, c, platform, raw); saveConfig(i.guildId, config, i.guild, actorId); setAccountSession(i, { creatorId: c.creatorId, platforms: [], accountId: r.accountId, routeType: 'default' }); return afterModal(i, 'creators', `✅ ${LABEL[platform]} account ${r.created ? 'added' : 'updated'}.`); }
