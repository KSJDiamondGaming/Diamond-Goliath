...
function buildSectionPanel(i, name) {
  ...
  if (name === 'accounts') {
    ...
    const components = []; if (creators.length) { components.push(creatorSelect(creators, session.creatorId, `${P}account:creator`, 'Select a creator profile to manage')); if (linked.length) components.push(accountSelect(linked, getAccountSession(i).accountId)); components.push(platformSelect(getAccountSession(i).platforms)); components.push(row(btn(`${P}creator:edit`, '📝 Edit Profile', ButtonStyle.Secondary, !creator), btn(`${P}account:continue`, '➕ Add Account', ButtonStyle.Success, !session.creatorId || !getAccountSession(i).platforms.length), btn(`${P}account:edit`, '🛠️ Manage Account', ButtonStyle.Primary, !active))); components.push(row(btn(`${P}account:creator:toggle`, creator?.enabled === false ? '▶️ Resume Profile' : '⏸️ Pause Profile', creator?.enabled === false ? ButtonStyle.Success : ButtonStyle.Secondary, !creator), btn(`${P}creator:delete`, '🗑️ Delete', ButtonStyle.Danger, !creator), btn(`${P}account:reset`, '🔄 Clear Selection'), btn(`${P}creators`, '⬅️ Creator Profiles'), btn(`${P}settings`, '⚙️ Settings'))); } else components.push(row(btn(`${P}creators`, '👥 Create Creator Profile', ButtonStyle.Primary))); return { embeds: [embed(config, '🛠️ Manage Creator', d, who(i), active ? platformColor(active.platform) : creator ? creatorAccent(linked) : null)], components };
  }
