...
function buildCreatorEditPanel(i, config, creator) {
  ...
  const components = [
    row(btn(`${P}creator:change`, '\u{1F4DD} Edit Details'), btn(`${P}creator:toggle`, creator.enabled === false ? '\u25B6\uFE0F Resume Profile' : '\u23F8\uFE0F Pause Profile', creator.enabled === false ? ButtonStyle.Success : ButtonStyle.Secondary), btn(`${P}creator:delete`, '\u{1F5D1}\uFE0F Delete', ButtonStyle.Danger)),
    row(btn(`${P}creator:accounts`, '\u2B05\uFE0F Back to Manage'), btn(`${P}settings`, '\u2699\uFE0F Settings')),
  ];
...
function buildSectionPanel(i, name) {
  ...
  if (name === 'accounts') {
    ...
    const components = []; if (creators.length) { components.push(creatorSelect(creators, session.creatorId, `${P}account:creator`, 'Select a creator profile to manage')); if (linked.length) components.push(accountSelect(linked, getAccountSession(i).accountId)); components.push(platformSelect(getAccountSession(i).platforms)); components.push(row(btn(`${P}creator:edit`, '📝 Edit Profile', ButtonStyle.Secondary, !creator), btn(`${P}account:continue`, '➕ Add Account', ButtonStyle.Success, !session.creatorId || !getAccountSession(i).platforms.length), btn(`${P}account:edit`, '🛠️ Manage Account', ButtonStyle.Primary, !active))); components.push(row(btn(`${P}account:creator:toggle`, creator?.enabled === false ? '▶️ Resume Profile' : '⏸️ Pause Profile', creator?.enabled === false ? ButtonStyle.Success : ButtonStyle.Secondary, !creator), btn(`${P}account:reset`, '🔄 Clear Selection'))); } else components.push(row(btn(`${P}creators`, '👥 Create Creator Profile', ButtonStyle.Primary))); components.push(row(btn(`${P}creators`, '⬅️ Creator Profiles'), btn(`${P}settings`, '⚙️ Settings'))); return { embeds: [embed(config, '🛠️ Manage Creator', d, who(i), active ? platformColor(active.platform) : creator ? creatorAccent(linked) : null)], components };
  }
...
function opensModal(id) { return id === `${P}creator:new` || id === `${P}creator:change` || id === `${P}account:continue` || id === `${P}account:change` || id === `${P}account:move:new` || id === `${P}automation:quiet` || (id.startsWith(`${P}template:`) && !id.startsWith(`${P}template:save:`)); }
