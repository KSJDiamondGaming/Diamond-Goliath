...
function alertTypeSelect(account) {
  ...
  return row(new StringSelectMenuBuilder().setCustomId(`${P}account:alerts`).setPlaceholder('Notifications to post for this account').setMinValues(0).setMaxValues(supported.length).addOptions(supported.map((type) => ({ label: ALERT_LABEL[type] || type, value: type, description: `Post ${ALERT_LABEL[type] || type} updates to Discord`, default: configured.includes(type) }))));
}
function routeTypeSelect(id, selected, types = ALERT_TYPES) {
  const copy = {
    default: { label: '🏠 Default Channel', description: 'All social posts go here unless you choose a special channel below.' },
...
function buildAccountEditPanel(i, config, creator, account) {
  ...
  const actions = [btn(`${P}account:check:${account.accountId}`, '🔄 Check Now', ButtonStyle.Primary), btn(`${P}account:change`, '📝 Edit Details'), btn(`${P}account:toggle`, account.enabled === false ? 'Resume' : 'Pause', account.enabled === false ? ButtonStyle.Success : ButtonStyle.Secondary)]; if (account.profileUrl && /^https?:\/\//i.test(account.profileUrl)) actions.push(linkBtn(account.profileUrl, '🔗 Profile')); actions.push(btn(`${P}account:delete`, '🗑️ Delete', ButtonStyle.Danger)); components.push(row(...actions.slice(0, 5))); components.push(row(btn(`${P}accounts`, 'Back to Creator Accounts'), btn(`${P}settings`, '⚙️ Settings')));
  ...
  const d = [`${ICON[account.platform]} **${LABEL[account.platform]} — ${account.username || account.externalId || 'Resolving…'}**`, '', `**Creator:** ${creator.displayName}`, `**Status:** ${accountState(account)}`, `**Last Checked:** ${ts(s.lastCheckedAt)}`, `**Alerts:** ${alerts.length ? alerts.map((t) => ALERT_LABEL[t] || t).join(', ') : 'None'}`, `**Default Channel:** ${account.alertChannelId ? `<#${account.alertChannelId}>` : config.alertsChannelId ? `Server default <#${config.alertsChannelId}>` : 'Not configured'}`, `**Special Channels:** ${routes || 'None'}`, `**Monitoring:** ${account.enabled === false ? '⏸️ Paused' : '🟢 Enabled'}`, ...(s.lastDeliveryError ? ['', `⚠️ **Last delivery:** ${String(s.lastDeliveryError).slice(0, 400)}`] : []), ...(s.lastError ? ['', `⚠️ **Provider:** ${String(s.lastError).slice(0, 400)}`] : [])].join('\n'); return { embeds: [embed(config, '🔗 Manage Social Account', d, who(i))], components: components.slice(0, 5) };
}
...
function buildSectionPanel(i, name) {
  ...
  if (name === 'accounts') {
    ...
    const components = []; if (creators.length) { components.push(creatorSelect(creators, session.creatorId)); if (linked.length) components.push(accountSelect(linked, getAccountSession(i).accountId)); components.push(platformSelect(getAccountSession(i).platforms)); components.push(row(btn(`${P}account:continue`, '➕ Add Account', ButtonStyle.Success, !session.creatorId || !getAccountSession(i).platforms.length), btn(`${P}account:edit`, '⚙️ Manage Selected', ButtonStyle.Primary, !active), btn(`${P}account:reset`, '↻ Reset'))); } else components.push(row(btn(`${P}creators`, '👥 Create Creator Profile', ButtonStyle.Primary))); components.push(row(btn(`${P}creators`, '⬅️ Creator Profiles'), btn(`${P}settings`, '⚙️ Settings'))); return { embeds: [embed(config, '🔗 Creator Accounts', d, who(i))], components };
  }
  if (name === 'notifications') return buildSectionPanel(i, 'operations');
  if (name === 'templates') { const c = [row(...ALERT_TYPES.slice(0, 5).map((t) => btn(`${P}template:${t}`, ALERT_LABEL[t], ButtonStyle.Primary))), row(...ALERT_TYPES.slice(5).map((t) => btn(`${P}template:${t}`, ALERT_LABEL[t], ButtonStyle.Primary)), btn(`${P}variables`, '🧩 Variables')), navigation('templates')]; return { embeds: [embed(config, '🎨 Alert Templates', 'Build rich notification embeds for each alert type. Templates support dynamic variables, platform colours, thumbnails, metadata fields and link buttons.\n\nUse **🧩 Variables** for the complete helper list.', who(i))], components: c }; }
  ...
  if (name === 'channels') { const session = getFeedSession(i), routeType = ALERT_TYPES.includes(session.routeType) ? session.routeType : 'default', selected = routeType === 'default' ? config.alertsChannelId : config.alertChannels?.[routeType]; const routeSummary = ALERT_TYPES.map((type) => `${ALERT_LABEL[type]}: ${config.alertChannels?.[type] ? `<#${config.alertChannels[type]}>` : 'Default channel'}`).join('\n'); const d = `Choose which Discord channels receive Social Studio posts.\n\n**🏠 Default Channel:** ${config.alertsChannelId ? `<#${config.alertsChannelId}>` : 'Not set'}\nEverything posts here unless you choose a separate channel below.\n\n**Special Channels**\n${routeSummary}\n\nPick what you want to configure, then choose the Discord channel.`; return { embeds: [embed(config, '📂 Channels', d, who(i))], components: [routeTypeSelect(`${P}channel:type`, routeType), channelSelect(`${P}channel:route`, selected, routeType === 'default' ? 'Choose the default channel' : `Choose where ${ALERT_LABEL[routeType]} posts go`), navigation('channels')] }; }
  if (name === 'settings') return { embeds: [embed(config, 'Social Studio Settings', 'Manage access, monitoring, live message behaviour and diagnostics.', who(i))], components: [row(btn(`${P}permissions`, 'Permissions', ButtonStyle.Primary), btn(`${P}monitoring`, 'Monitoring', ButtonStyle.Primary), btn(`${P}liveMessages`, 'Live Messages', ButtonStyle.Primary), btn(`${P}diagnostics`, 'Diagnostics', ButtonStyle.Primary)), navigation('settings')] };
  if (name === 'permissions') { const configuredRoles = config.managerRoleIds.length ? config.managerRoleIds.map((id) => `<@&${id}>`).join(', ') : 'None'; const d = ['Control who is allowed to manage Social Studio.', '', '**Access model**', '- Server administrators retain access through the admin panel.', '- Additional Social Studio managers are granted through the manager roles selected below.', '- Alert recipients and mention roles are configured per account and are separate from management access.', '', `**Manager roles:** ${configuredRoles}`, '', 'Select zero roles to remove all additional Social Studio managers.'].join('\n'); return { embeds: [embed(config, 'Permissions', d, who(i))], components: [roleSelect(config.managerRoleIds), navigation('permissions')] }; }
  ...
  if (name === 'operations') return { embeds: [embed(config, 'Operations', 'Choose the area you want to manage.', who(i))], components: [row(btn(`${P}monitoring`, 'Monitoring', ButtonStyle.Primary), btn(`${P}liveMessages`, 'Live Messages', ButtonStyle.Primary), btn(`${P}diagnostics`, 'Diagnostics', ButtonStyle.Primary)), navigation('operations')] };
  if (name === 'monitoring') {
    ...
    return { embeds: [embed(config, 'Monitoring', d, who(i))], components: [row(btn(`${P}toggle`, config.enabled ? 'Disable Module' : 'Enable Module', config.enabled ? ButtonStyle.Danger : ButtonStyle.Success), btn(`${P}account:check`, 'Check Providers', ButtonStyle.Primary, !accounts.length)), row(btn(`${P}automation:interval`, 'Interval'), btn(`${P}automation:dupes`, settings.suppressDuplicates === false ? 'Duplicates: Allowed' : 'Duplicates: Blocked'), btn(`${P}automation:retry`, settings.retryDeliveries === false ? 'Retry: Off' : 'Retry: On'), btn(`${P}automation:quiet`, quiet.enabled === true ? 'Quiet: On' : 'Quiet: Off')), row(btn(`${P}operations`, 'Operations'), btn(`${P}settings`, 'Settings'))] };
  }
  if (name === 'liveMessages') {
    ...
    return { embeds: [embed(config, 'Live Messages', d, who(i))], components: [row(btn(`${P}automation:editlive`, settings.editLiveNotifications === true ? 'Edit: On' : 'Edit: Off'), btn(`${P}automation:deleteended`, settings.deleteEndedNotifications === true ? 'Cleanup: On' : 'Cleanup: Off'), btn(`${P}automation:viewers`, settings.includeViewerCount === false ? 'Viewers: Off' : 'Viewers: On'), btn(`${P}automation:duration`, settings.includeLiveDuration === false ? 'Duration: Off' : 'Duration: On')), row(btn(`${P}operations`, 'Operations'), btn(`${P}settings`, 'Settings'))] };
  }
  if (name === 'diagnostics') {
    ...
    const d = ['**Testing & Data**', `Default channel: ${config.alertsChannelId ? `<#${config.alertsChannelId}>` : 'Not configured'}`, `Accounts: ${accounts.length} (${monitored} monitored)`, `Provider checks: ${checks.toLocaleString('en-GB')}`, `Alerts sent: ${alerts.toLocaleString('en-GB')}`, `Failures: ${failures.toLocaleString('en-GB')}`, `Queue size: ${config.queue.length}`, `History entries: ${config.history.length}`, `Last successful scan: ${ts(lastSuccess?.createdAt)}`, `Last failure: ${ts(lastFailure?.createdAt)}`, '', '**Recent Activity**', recent].join('\n');
    return { embeds: [embed(config, 'Diagnostics', d, who(i))], components: [row(btn(`${P}test`, 'Send Test', ButtonStyle.Primary, !config.alertsChannelId), btn(`${P}testing:last`, 'Last Response'), btn(`${P}testing:diagnostics`, 'Provider Details')), row(btn(`${P}data:export`, 'Export', ButtonStyle.Primary), btn(`${P}data:clear`, 'Clear History', ButtonStyle.Danger, !config.history.length)), row(btn(`${P}operations`, 'Operations'), btn(`${P}settings`, 'Settings'))] };
  }
...
async function handleInteraction(i) {
  ...
  if (id === `${P}data:clear`) return respond(i, { embeds: [embed(config, '⚠️ Clear Social Studio History', `This will remove **${config.history.length}** stored history entries. Account configuration and monitoring state will not be deleted.`, who(i))], components: [row(btn(`${P}data:clear:cancel`, 'Cancel'), btn(`${P}data:clear:confirm`, 'Clear History', ButtonStyle.Danger))] });
