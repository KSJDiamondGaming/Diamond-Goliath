...
function buildSectionPanel(i, name) {
  ...
  if (name === 'templates') { const c = [row(...ALERT_TYPES.slice(0, 5).map((t) => btn(`${P}template:${t}`, `${ALERT_EMOJI[t] || '🔔'} ${ALERT_LABEL[t]}`, ButtonStyle.Primary))), row(...ALERT_TYPES.slice(5).map((t) => btn(`${P}template:${t}`, `${ALERT_EMOJI[t] || '🔔'} ${ALERT_LABEL[t]}`, ButtonStyle.Primary)), btn(`${P}variables`, '🧩 Variables')), navigation('templates')]; return { embeds: [embed(config, '🎨 Alert Templates', 'Build rich notification embeds for each alert type. Templates support dynamic variables, platform colours, thumbnails, metadata fields and link buttons.\n\nUse **🧩 Variables** for the complete helper list.', who(i))], components: c }; }
  ...
  if (name === 'diagnostics') {
    ...
    return { embeds: [embed(config, '🧪 Diagnostics', d, who(i))], components: [row(btn(`${P}test`, '📨 Send Test', ButtonStyle.Primary, !config.alertsChannelId), btn(`${P}testing:last`, '📄 Last Response'), btn(`${P}testing:diagnostics`, '🩺 Provider Details')), row(btn(`${P}data:export`, '📤 Export', ButtonStyle.Primary), btn(`${P}data:clear`, '🧹 Clear History', ButtonStyle.Danger, !config.history.length)), row(btn(`${P}operations`, '⚙️ Operations'), btn(`${P}settings`, '⚙️ Settings'))] };
  }
