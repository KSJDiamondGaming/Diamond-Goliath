...
function routeTypeSelect(id, selected, types = ALERT_TYPES) {
  const options = [{ label: 'Default route', value: 'default', description: 'Fallback channel for this scope' }, ...types.map((type) => ({ label: ALERT_LABEL[type] || type, value: type, description: `Route ${ALERT_LABEL[type] || type} alerts` }))];
  return row(new StringSelectMenuBuilder().setCustomId(id).setPlaceholder('Choose alert route to configure').setMinValues(1).setMaxValues(1).addOptions(options.map((option) => ({ ...option, default: option.value === selected }))));
}
...
function dashboardStats(config) {
  ...
  return {
    live: accounts.filter((a) => a.enabled !== false && a.state?.isLive === true).length,
    offline: accounts.filter((a) => a.enabled !== false && a.state?.isLive === false).length,
    unavailable: accounts.filter((a) => a.enabled !== false && a.state?.lastError).length,
    monitored: accounts.filter((a) => a.enabled !== false).length,
  };
}
...
function buildMainPanel(guild, requestedBy = 'Unknown User') {
  ...
  const d = [
    `${ready ? '✅' : '⚠️'} **${ready ? 'Social Studio is ready.' : 'Setup required'}**`, '',
    `**Creators:** ${creators}  •  **Accounts:** ${accounts}`,
    `🔴 **LIVE:** ${stats.live}  •  ⚫ **Offline:** ${stats.offline}  •  🟡 **Issues:** ${stats.unavailable}`,
    `📡 **Monitoring:** ${stats.monitored}/${accounts}`,
    `📨 **Alerts Sent:** ${Number(c.analytics?.alertsSent || 0).toLocaleString('en-GB')}`,
    `📂 **Default Feed:** ${c.alertsChannelId ? `<#${c.alertsChannelId}>` : 'Not configured'}`,
    `🔔 **Notifications:** ${c.enabled ? '🟢 Enabled' : '🔴 Disabled'}`,
  ].join('\n');
...
function buildAccountEditPanel(i, config, creator, account) {
  ...
  const alertMenu = alertTypeSelect(account); if (alertMenu) components.push(alertMenu);
  components.push(routeTypeSelect(`${P}account:route:type`, routeType, supported));
  const routeChannelId = routeType === 'default' ? account.alertChannelId : account.alertChannels?.[routeType];
  components.push(channelSelect(`${P}account:route:channel`, routeChannelId, routeType === 'default' ? 'Account default notification channel' : `${ALERT_LABEL[routeType]} notification channel`));
  const actions = [btn(`${P}account:check:${account.accountId}`, '🔄 Check Now', ButtonStyle.Primary), btn(`${P}account:change`, '📝 Edit Details'), btn(`${P}account:toggle`, account.enabled === false ? '▶️ Resume' : '⏸️ Pause', account.enabled === false ? ButtonStyle.Success : ButtonStyle.Secondary)];
  if (account.profileUrl && /^https?:\/\//i.test(account.profileUrl)) actions.push(linkBtn(account.profileUrl, '🔗 Profile'));
  actions.push(btn(`${P}account:delete`, '🗑️ Delete', ButtonStyle.Danger));
  components.push(row(...actions.slice(0, 5)));
  ...
  const d = [`${ICON[account.platform]} **${LABEL[account.platform]} — ${account.username || account.externalId || 'Resolving…'}**`, '', `**Creator:** ${creator.displayName}`, `**Feed State:** ${accountState(account)}`, `**Last Checked:** ${checked}`, `**Alerts:** ${alerts.length ? alerts.map((t) => ALERT_LABEL[t] || t).join(', ') : 'None'}`, `**Default Route:** ${account.alertChannelId ? `<#${account.alertChannelId}>` : config.alertsChannelId ? `Server default <#${config.alertsChannelId}>` : 'Not configured'}`, `**Custom Routes:** ${routes || 'None'}`, `**Monitoring:** ${account.enabled === false ? '⏸️ Paused' : '🟢 Enabled'}`, ...(s.lastDeliveryError ? ['', `⚠️ **Last delivery:** ${String(s.lastDeliveryError).slice(0, 400)}`] : []), ...(s.lastError ? ['', `⚠️ **Provider:** ${String(s.lastError).slice(0, 400)}`] : [])].join('\n'); return { embeds: [embed(config, '🔗 Manage Social Account', d, who(i))], components: components.slice(0, 5) };
}

function variablesDescription() {
  return [
    '**🌍 Global / Server**',
    '`{timestamp}` `{nowTimestamp}` `{guildId}` `{guildName}` `{server}` `{guildIcon}` `{serverIcon}` `{guildBanner}` `{guildMemberCount}` `{memberCount}` `{guildVanityCode}`',
    '`{successEmoji}` `{warningEmoji}` `{errorEmoji}` `{proofVerifiedEmoji}` `{successColor}` `{warningColor}` `{errorColor}` `{proofVerifiedColor}`', '',
    '**👤 Discord User Context**',
    '`{userId}` `{userTag}` `{userName}` `{userGlobalName}` `{userMention}` `{userNoPing}` `{userAvatar}` `{userServerAvatar}` `{userNickname}` `{userDisplay}`',
    '`{userCreatedAt}` `{userCreatedTimestamp}` `{userJoinedAt}` `{userJoinedTimestamp}` `{createdAt}` `{joinedAt}` `{leftAt}` `{accountAge}` `{membershipDuration}`',
    '`{departureIcon}` `{departureType}` `{departureLabel}` `{departureReason}` `{departureModerator}` `{departureModeratorId}`', '',
    '**📣 Creator / Platform**',
    '`{creator}` `{creatorName}` `{creatorDisplayName}` `{creatorAvatar}` `{creatorBanner}` `{creatorDescription}` `{platform}` `{platformIcon}` `{platformColor}` `{username}` `{displayName}` `{channelId}` `{profileUrl}`', '',
    '**🔴 LIVE / Stream**',
    '`{title}` `{description}` `{game}` `{category}` `{viewers}` `{peakViewers}` `{started}` `{duration}` `{liveThumbnail}` `{thumbnail}` `{liveUrl}` `{url}`', '',
    '**🎥 Video / VOD / Upload / Clip / Short**',
    '`{videoTitle}` `{videoDescription}` `{videoDuration}` `{videoViews}` `{videoThumbnail}` `{videoUrl}`',
    '`{clipTitle}` `{clipCreator}` `{clipViews}` `{clipUrl}` `{uploadTitle}` `{uploadDescription}` `{uploadThumbnail}` `{uploadUrl}` `{shortTitle}` `{shortThumbnail}` `{shortUrl}`', '',
...
function buildSectionPanel(i, name) {
  ...
  if (name === 'notifications') {
    const active = accounts.filter((a) => a.enabled !== false).length, enabledAlerts = accounts.reduce((n, a) => n + (Array.isArray(a.alertTypes) ? a.alertTypes.length : supportedAlerts(a.platform).length), 0);
    return { embeds: [embed(config, '📢 Notifications', `**Module:** ${config.enabled ? '🟢 Enabled' : '🔴 Disabled'}\n**Monitored Accounts:** ${active}/${accounts.length}\n**Enabled Alert Types:** ${enabledAlerts}\n**Default Channel:** ${config.alertsChannelId ? `<#${config.alertsChannelId}>` : 'Not configured'}\n**Alerts Sent:** ${Number(config.analytics?.alertsSent || 0).toLocaleString('en-GB')}\n\nAccount-specific and alert-type routes override the default feed.`, who(i))], components: [row(btn(`${P}toggle`, config.enabled ? '⏸️ Disable Notifications' : '▶️ Enable Notifications', config.enabled ? ButtonStyle.Danger : ButtonStyle.Success), btn(`${P}account:check`, '🔄 Check Providers', ButtonStyle.Primary, !accounts.length), btn(`${P}feeds`, '📡 Configure Feeds')), navigation('notifications')] };
  }
  if (name === 'templates') {
    const c = [row(...ALERT_TYPES.slice(0, 5).map((t) => btn(`${P}template:${t}`, ALERT_LABEL[t], ButtonStyle.Primary))), row(...ALERT_TYPES.slice(5).map((t) => btn(`${P}template:${t}`, ALERT_LABEL[t], ButtonStyle.Primary)), btn(`${P}variables`, '🧩 Variables')), navigation('templates')];
  ...
  if (name === 'feeds') {
    ...
    const routeSummary = ALERT_TYPES.map((type) => `${ALERT_LABEL[type]}: ${config.alertChannels?.[type] ? `<#${config.alertChannels[type]}>` : 'Default'}`).join('\n');
    const d = `Route each type of Social Studio alert to the right Discord channel.\n\n**Default:** ${config.alertsChannelId ? `<#${config.alertsChannelId}>` : 'Not configured'}\n\n${routeSummary}`;
...
async function handleInteraction(i) {
  ...
  if (id === `${P}feed:type`) { setFeedSession(i, { routeType: i.values?.[0] || 'default' }); return respond(i, buildSectionPanel(i, 'feeds')); }
  if (id === `${P}feed:route`) { const type = getFeedSession(i).routeType || 'default', channelId = i.values?.[0] || null; if (type === 'default') config.alertsChannelId = channelId; else { config.alertChannels = config.alertChannels && typeof config.alertChannels === 'object' ? config.alertChannels : {}; config.alertChannels[type] = channelId; } saveConfig(i.guildId, config, i.guild, actorId); return respond(i, buildSectionPanel(i, 'feeds')); }
  ...
  if (id === `${P}automation:interval`) { const values = [60000, 120000, 300000, 600000, 900000], current = Math.max(60000, Number(config.settings?.checkIntervalMs || 300000)), index = values.findIndex((v) => v >= current), next = values[(index < 0 ? 0 : index + 1) % values.length]; config.settings = { ...(config.settings || {}), checkIntervalMs: next }; saveConfig(i.guildId, config, i.guild, actorId); return respond(i, buildSectionPanel(i, 'automation')); }
  if (id === `${P}automation:dupes`) { config.settings = { ...(config.settings || {}), suppressDuplicates: config.settings?.suppressDuplicates === false }; saveConfig(i.guildId, config, i.guild, actorId); return respond(i, buildSectionPanel(i, 'automation')); }
