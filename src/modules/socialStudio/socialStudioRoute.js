...
function getConfig(id) { return normalize(guildManager.getGuildSection(id, 'social', defaults())); }
function saveConfig(id, config, meta = {}) {
  const next = normalize({ ...config, updatedAt: now(), lastActorId: meta.actorId || null });
  guildManager.saveGuildSection(id, 'social', next, { guildId: id });
  guildManager.setModuleEnabled(id, 'social', next.enabled, { guildId: id });
  return next;
}
...
router.patch('/:guildId/config', (req, res) => { try { const id = guildId(req); const current = getConfig(id); const body = isObject(req.body) ? req.body : {}; const config = { ...current, ...body, settings: { ...current.settings, ...(isObject(body.settings) ? body.settings : {}) }, templates: { ...current.templates, ...(isObject(body.templates) ? body.templates : {}) } }; return success(res, { guildId: id, config: saveConfig(id, config, actor(req)) }); } catch (error) { return failure(res, error, 400); } });
