...
async function upsertStarboardPost(client, message, section, starUserIds) { /* unchanged */
  const starboardChannel = message.guild.channels.cache.get(section.channelId) || await message.guild.channels.fetch(section.channelId).catch(() => null);
  ...
  if (existing?.starboardMessageId) {
    ...
    if (starboardMessage?.editable) {
      ...
      return starboardStore.savePost(message.guild.id, { ...existing, starUserIds });
    }
  }
  const sent = await starboardChannel.send({ content: `⭐ **${starUserIds.length}** <#${message.channel.id}>`, embeds: [embed] });
  return starboardStore.savePost(message.guild.id, { messageId: message.id, channelId: message.channel.id, authorId: message.author?.id, starboardMessageId: sent.id, starUserIds });
}

async function removeStarboardPost(client, message, section) { const existing = starboardStore.getPost(message.guild.id, message.id); if (!existing?.starboardMessageId) return null; const starboardChannel = message.guild.channels.cache.get(section.channelId) || await message.guild.channels.fetch(section.channelId).catch(() => null); const starboardMessage = await starboardChannel?.messages?.fetch(existing.starboardMessageId).catch(() => null); if (starboardMessage?.deletable) await starboardMessage.delete().catch(() => null); starboardStore.deletePost(message.guild.id, message.id); return existing; }
...
async function handleStarReactionRemove(reaction, user, client) {
  ...
  const starUserIds = await getStarUsers(reaction);
  if (starUserIds.length < section.threshold) return removeStarboardPost(client, message, section);
  return upsertStarboardPost(client, message, section, starUserIds);
}
...
function configureStarboard(guildId, input = {}) {
  ...
  return starboardStore.updateStarboardSection(guildId, (section) => ({ ...section, enabled: input.enabled ?? section.enabled, channelId: input.channelId ?? section.channelId, threshold: input.threshold ?? section.threshold, emoji: input.emoji ?? section.emoji, allowBotMessages: input.allowBotMessages ?? section.allowBotMessages, allowSelfStar: input.allowSelfStar ?? section.allowSelfStar, updatedAt: starboardStore.now() }));
}

module.exports = { canManageStarboard, buildStarboardEmbed, configureStarboard, handleStarReactionAdd, handleStarReactionRemove };
