...
const fail = (res, error, status = 400) => res.status(status).json({ success: false, error: error.message || 'Timed Roles request failed.' });
const guildId = (req) => {
...
router.put('/:guildId/rules/:ruleId', async (req, res) => {
  try {
    ...
    const rule = timedRoles.saveRule(id, { ...current, ...(req.body || {}), ruleId: current.ruleId }, { actorId: actor(req) });
    return ok(res, { rule, ...(await overview(req, id)) });
  } catch (error) { return fail(res, error); }
});
...
router.post('/:guildId/repair', async (req, res) => {
  try {
    ...
    const target = await guild(req, id);
    if (!target) throw new Error('Guild is unavailable.');
    timedRoles.repair(target, { actorId: actor(req) });
    return ok(res, await overview(req, id));
  } catch (error) { return fail(res, error); }
});
