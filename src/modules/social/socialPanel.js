...
async function handleSocialAdminInteraction(interaction) {
  ...
  try {
    ...
    if (customId === 'admin:social:enable') social.setEnabled(interaction.guild.id, true, interaction.guild);
    if (customId === 'admin:social:disable') social.setEnabled(interaction.guild.id, false, interaction.guild);
    ...
    if (customId === 'admin:social:createTest') {
      ...
      const defaults = getAdminDefaults(section);
      social.addAccount(interaction.guild.id, {
        platform: 'twitch',
    ...
    if (customId === 'admin:social:sendTest') {
      ...
      const result = await social.sendTestAlert(interaction.guild.id, accounts[0].accountId, interaction.client, { actorId: interaction.user.id });
