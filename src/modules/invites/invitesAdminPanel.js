...
function goliathView(interaction) {
  ...
  return {
    embeds: [new EmbedBuilder()
      ...
      .setDescription('Configure official Goliath links and the one-click member invite template.')
      .addFields(
    ...
    components: [
      row(
        button('invites:official-settings', 'Official Invite Settings', ButtonStyle.Primary),
        button('invites:member-settings', 'Member Link Settings', ButtonStyle.Primary),
      ),
...
function publicSettingsView(interaction) {
  ...
  return {
    embeds: [new EmbedBuilder()
      ...
      .setTitle('📣 Configure Public Panel')
      .setDescription('Choose where the public invite and member leaderboard panel is deployed.')
      .addFields(
        { name: 'Channel', value: config.channelId ? `<#${config.channelId}>` : 'Not selected', inline: true },
        ...
        { name: 'Official Invite', value: section.settings.officialInvite.code ? 'Ready' : 'Not selected', inline: true },
      )],
    components: [
      row(new ChannelSelectMenuBuilder()
...
async function handleInviteStudioInteraction(interaction) {
  ...
  if (action === 'home') state.page = 'overview';
    ...
    await interaction.editReply(result.valid
      ? `✅ Selected official link is valid and matches these settings: ${result.invite.url}`
