...
function textInput(id, label, style, value, maxLength) {
  ...
  const cleanValue = String(value ?? '').slice(0, maxLength);
  if (cleanValue) input.setValue(cleanValue);
  return input;
}
...
function getPanelConfigSafe(guildId) {
  try {
    return publicPanels.panelConfig(guildId);
  } catch (error) {
    console.error('[InviteStudio] Failed to read public panel configuration:', error);
    return {
      publicPanel: { ...publicPanels.DEFAULT_PUBLIC },
      leaderboardPanel: { ...publicPanels.DEFAULT_LEADERBOARD },
    };
  }
}
...
function overview(interaction) {
  ...
  const config = getPanelConfigSafe(interaction.guildId);

  const embed = new EmbedBuilder()
    .setColor(section.enabled ? 0x57F287 : 0xED4245)
    .setTitle('📨 Invite Studio')
    .setDescription('Create private invite links, deploy a public invite embed, and maintain a live leaderboard.')
    .addFields(
  ...
  return {
    ...
    components: [
      row(
        button('invites:create', 'Create Invite Link', ButtonStyle.Primary),
        button('invites:links', 'Invite Links'),
        button('invites:public', 'Public Invite Panel'),
      ),
      row(
        button('invites:leaderboard', 'Leaderboard Panel'),
        button('invites:sync', 'Sync Invites'),
...
function createView(interaction) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription('This link is only shown to you after creation. You can later select a permanent link for the public panel.')
    .addFields(
  ...
  return {
    ...
    components: [
      row(new ChannelSelectMenuBuilder()
        .setCustomId('invites:draft-channel')
        .setPlaceholder('Select invite channel')
        .addChannelTypes(ChannelType.GuildText)),
      row(new StringSelectMenuBuilder()
        .setCustomId('invites:draft-expiry')
        .setPlaceholder(`Expire after: ${expiryLabel(draft.maxAge)}`)
        .addOptions(
      ...
      row(new StringSelectMenuBuilder()
        .setCustomId('invites:draft-uses')
        .setPlaceholder(`Max uses: ${draft.maxUses || 'No limit'}`)
        .addOptions(
      ...
      row(new RoleSelectMenuBuilder()
        .setCustomId('invites:draft-roles')
        .setPlaceholder('Roles (optional)')
        .setMinValues(0)
        .setMaxValues(10)),
      row(
        button('invites:draft-temporary', draft.temporary ? 'Temporary Membership: On' : 'Temporary Membership: Off', draft.temporary ? ButtonStyle.Success : ButtonStyle.Secondary),
        button('invites:generate', 'Generate Invite', ButtonStyle.Primary, !draft.channelId),
        button('invites:home', 'Cancel'),
      ),
    ],
  };
}
...
function linksView(interaction) {
  ...
  return {
    ...
    components: [row(
      button('invites:create', 'Create Invite Link', ButtonStyle.Primary),
      button('invites:delete', 'Delete Link', ButtonStyle.Danger, !links.length),
...
function publicPanelView(interaction) {
  const config = getPanelConfigSafe(interaction.guildId).publicPanel || publicPanels.DEFAULT_PUBLIC;
  const links = invites.listInviteLinks(interaction.guildId).filter((link) => link.maxAge === 0 && link.maxUses === 0);
  const embed = new EmbedBuilder()
    .setColor(safeColor(config.color))
    .setTitle('📣 Public Invite Panel')
    .setDescription('Choose a permanent Invite Studio link, customise the embed, then deploy or update one public message.')
    .addFields(
      ...
      { name: 'Message', value: config.messageId ? 'Deployed' : 'Not deployed', inline: true },
      { name: 'Title', value: safeText(config.title, 'Help Grow the Community', 1024), inline: false },
      { name: 'Description', value: safeText(config.description, 'Share our official invite link.', 1000), inline: false },
    );

  const components = [row(new ChannelSelectMenuBuilder()
    .setCustomId('invites:public-channel')
  ...
  if (links.length) {
    components.push(row(new StringSelectMenuBuilder()
      ...
      .addOptions(links.slice(0, 25).map((link) => ({
        label: safeText(link.code, 'Invite', 100),
        description: `${link.uses || 0} uses • ${link.roleIds?.length || 0} role(s)`.slice(0, 100),
        value: link.code,
      }))));
  }

  components.push(row(
    button('invites:public-edit', 'Edit Embed', ButtonStyle.Primary),
    button('invites:public-deploy', config.messageId ? 'Update Public Panel' : 'Deploy Public Panel', ButtonStyle.Success, !config.channelId || !config.inviteCode),
    button('invites:home', 'Back'),
  ));
...
function leaderboardPanelView(interaction) {
  const config = getPanelConfigSafe(interaction.guildId).leaderboardPanel || publicPanels.DEFAULT_LEADERBOARD;
  const embed = new EmbedBuilder()
    .setColor(safeColor(config.color))
    .setTitle('🏆 Leaderboard Panel')
    .setDescription('Deploy one editable leaderboard message. It updates automatically when tracked members join or leave.')
    .addFields(
      { name: 'Channel', value: config.channelId ? `<#${config.channelId}>` : 'Not selected', inline: true },
      { name: 'Message', value: config.messageId ? 'Deployed' : 'Not deployed', inline: true },
      { name: 'Entries shown', value: String(config.limit || 10), inline: true },
      { name: 'Title', value: safeText(config.title, 'Community Invite Leaderboard', 1024), inline: false },
      { name: 'Description', value: safeText(config.description, 'Invite friends and climb the rankings.', 1024), inline: false },
    );
  ...
  return {
    ...
    components: [
      row(new ChannelSelectMenuBuilder()
        .setCustomId('invites:leaderboard-channel')
        .setPlaceholder('Select leaderboard channel')
        .addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement)),
      row(
        button('invites:leaderboard-edit', 'Edit Embed', ButtonStyle.Primary),
        button('invites:leaderboard-deploy', config.messageId ? 'Update Leaderboard' : 'Deploy Leaderboard', ButtonStyle.Success, !config.channelId),
        button('invites:leaderboard-refresh', 'Refresh Now', ButtonStyle.Secondary, !config.messageId),
        button('invites:home', 'Back'),
      ),
    ],
  };
...
function publicEditModal(guildId) {
  const config = getPanelConfigSafe(guildId).publicPanel || publicPanels.DEFAULT_PUBLIC;
  return new ModalBuilder()
    .setCustomId('invites:public-edit-modal')
    .setTitle('Edit Public Invite Embed')
    .addComponents(
...
function leaderboardEditModal(guildId) {
  const config = getPanelConfigSafe(guildId).leaderboardPanel || publicPanels.DEFAULT_LEADERBOARD;
  return new ModalBuilder()
    .setCustomId('invites:leaderboard-edit-modal')
    .setTitle('Edit Leaderboard Embed')
    .addComponents(
      row(textInput('title', 'Title', TextInputStyle.Short, config.title, 256)),
...
async function respondWithPanel(interaction) {
...
async function handleInviteStudioInteraction(interaction) {
  ...
  if (['create', 'links', 'public', 'leaderboard'].includes(action)) {
    state.page = action;
    if (action === 'create') {
      state.draft = { channelId: interaction.channelId || null, roleIds: [], maxAge: 2592000, maxUses: 0, temporary: false };
    }
    await respondWithPanel(interaction);
    return true;
  }
  ...
  if (action === 'health') {
    ...
    await interaction.reply({
      content: health.healthy ? '✅ Invite Studio is healthy.' : `⚠️ ${health.issues.length} issue(s), ${health.warnings.length} warning(s).`,
  ...
  if (action === 'draft-channel' && interaction.isChannelSelectMenu()) state.draft.channelId = interaction.values[0];
  ...
  else if (action === 'public-edit') {
    await interaction.showModal(publicEditModal(interaction.guildId));
    return true;
  } else if (action === 'leaderboard-edit') {
    await interaction.showModal(leaderboardEditModal(interaction.guildId));
    return true;
  } else if (action === 'public-edit-modal' && interaction.isModalSubmit()) {
    publicPanels.savePanelConfig(interaction.guildId, 'publicPanel', {
      title: modalValue(interaction, 'title'),
      description: modalValue(interaction, 'description'),
      color: modalValue(interaction, 'color'),
      footer: modalValue(interaction, 'footer'),
      buttonLabel: modalValue(interaction, 'buttonLabel'),
    }, meta);
    await interaction.reply({ content: '✅ Public invite embed saved.', flags: MessageFlags.Ephemeral });
    return true;
  } else if (action === 'leaderboard-edit-modal' && interaction.isModalSubmit()) {
    publicPanels.savePanelConfig(interaction.guildId, 'leaderboardPanel', {
      title: modalValue(interaction, 'title'),
      description: modalValue(interaction, 'description'),
      color: modalValue(interaction, 'color'),
      footer: modalValue(interaction, 'footer'),
      limit: Math.max(3, Math.min(25, Number(modalValue(interaction, 'limit')) || 10)),
    }, meta);
    await interaction.reply({ content: '✅ Leaderboard embed saved.', flags: MessageFlags.Ephemeral });
    return true;
    ...
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    const refreshed = await publicPanels.refreshLeaderboard(interaction.guild);
    await interaction.editReply(refreshed ? '✅ Leaderboard refreshed.' : '❌ The deployed leaderboard message could not be found.');
    return true;
    ...
    const modal = new ModalBuilder()
      ...
      .addComponents(row(new TextInputBuilder()
        ...
        .setRequired(true)
        .setPlaceholder(links[0]?.code || 'abc123')));
